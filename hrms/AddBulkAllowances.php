...
{	
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
		...
		{	
			...
			$EmpID = $EmpID['ID'];
			$validate="SELECT ID from allowances WHERE ID <> 0 AND EmpID = '".$EmpID."' AND Title = '".$data[2]."'";
			$result = mysql_query($validate) or die(mysql_error());
			$num = mysql_num_rows($result);
			if($num == 0)
			{
				$import="INSERT into allowances(EmpID,FrwdEmpID,Title,Amount,Percentage,Type,Taxable,Date) values('$EmpID','$fwdEmpID','$data[2]','$data[3]','$data[3]','$data[4]','$data[5]','$data[6]')";
    ...
    $_SESSION["msg"]='<div class="alert alert-success alert-dismissable">
		...
		<b>Allowances has been Uploaded.</b>
		</div>';
	}
	else
	{
		...
		<i class="fa fa-ban"></i>
		<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
		<b>Allowances Not Uploaded.</b>
		</div>';
	}

		redirect("AddBulkAllowances.php");	
	}
...
<head>
<meta charset="UTF-8">
<title>Add Bulk Allowances</title>
            ...
            <div style="padding:15px;" class="box-primary">
              ...
              <div class="box-body">
				...
				<div class="form-group">
                  <label id="labelimp" for="Allowances" class="labelimp" >Upload (CSV): </label>
                  <input type="file" id="Allowances" name="flPage" />
                </div>
                <input type="hidden" name="action" value="submit_form" />
              </div>
              <!-- /.box-body -->
