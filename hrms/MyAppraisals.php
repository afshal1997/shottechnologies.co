...
<!--?php
if($_SESSION['RoleID'] == 'Administrator' OR $_SESSION['RoleID'] == 'HR' OR user_authentication($_SESSION['UserID'],'TrainExeMyTrangs') OR external_user_authentication($_SESSION['UserID'],'TrainExeMyTrangs'))
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT ID,Title,StartTime,EndTime,StartDate,EndDate FROM appraisals  WHERE ID <> 0 AND FIND_IN_SET (".$_SESSION['UserID'].", Employees)";
				if($Keywords != "")
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1>My Appraisals <small></small> </h1>
      <ol class="breadcrumb">
      ...
      <div class="row">
		...
		?>
                    <tr>
					  ...
					  <td>
					  <?php 
							$sql2 = "SELECT ID, EmpID, EmpMarks FROM appraisals_result WHERE EmpID = '".$_SESSION['UserID']."' AND AppraisalID = '".$row['ID']."'";
							$da2 = mysql_query($sql2);
							$ro2 = mysql_fetch_assoc($da2);
							if($ro2['EmpMarks'] > 0)
							{
								echo '<span class="btn" style="background:green; color:#fff;">Rated</span>';
							}
							else{
								...
								echo ((date('Y-m-d', strtotime(date('Y-m-d'))) >= date('Y-m-d', strtotime($strDate))) && (date('Y-m-d', strtotime(date('Y-m-d'))) <= date('Y-m-d', strtotime($enDate))) ? '<a class="btn btn-primary" href="GiveRank.php?ID='.$row['ID'].'">Rate Yourself</a>' : '<span class="btn btn-danger">'.((date('Y-m-d', strtotime(date('Y-m-d'))) > date('Y-m-d', strtotime($enDate))) ? 'You Missed' : 'Pending').'</span>');
							}
							?>
					  </td>
                    </tr>
