...
{			
	...
	if(isset($_POST["Dates"]) && $_POST["Dates"] != "")
	{
		...
		if($Type == "Auto Leaves")
		{
			$query="SELECT AnualLeaves,CasualLeaves,SickLeaves FROM current_leaves_quota WHERE EmpID = ".$_SESSION['UserID']."";
			$res = mysql_query($query) or die (mysql_error());
			$num = mysql_num_rows($res);
			if($num == 1)
			{
				$r = mysql_fetch_array($res);
				$num_of_available_days = ($r['AnualLeaves'] + $r['CasualLeaves'] + $r['SickLeaves']);
