...
$query="SELECT Step1ID,Step2ID,Step3ID,DATE_FORMAT(Step1Date, '%D %b %Y') AS Step1Date,DATE_FORMAT(Step2Date, '%D %b %Y') AS Step2Date,DATE_FORMAT(Step3Date, '%D %b %Y') AS Step3Date,Steps,MonthEncashment,CompanyID,EncashmentDate,Heading,Remarks,DATE_FORMAT(DateAdded, '%D %b %Y') AS Created FROM encashment WHERE ID='" . (int)$ID . "'";
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> Encashment Sheet<small></small> </h1>
      <ol class="breadcrumb">
        ...
        <li class="active">Encashment Sheet</li>
      </ol>
    </section>
	...
	<div class="box-footer no-print" style="text-align:right;">
				 ...
				 <a href="EncashmentSheetExcel.php?ID=<?php echo $ID; ?>" class="btn btn-primary margin no-print">Excel Sheet</a>
				 <?php 
				 if($_SESSION['RoleID'] == 'HR' OR $_SESSION['RoleID'] == 'Administrator'){
				 if($Steps == 0){ 
				 ?>
				 <a class="btn btn-success margin" href="HREncashmentOkay.php?ID=<?php echo $ID; ?>">Submit to Accounts <i class="fa fa-arrow-right"></i></a>
				 <?php }
				 }				 
				 if($_SESSION['RoleID'] == 'Accounts'){
				 if($Steps == 1){
				 ?>
				 <a class="btn btn-danger margin" href="HREncashmentBack.php?ID=<?php echo $ID; ?>"><i class="fa fa-arrow-left"></i> Back to HR</a>
				 <a class="btn btn-success margin" href="AccountsEncashmentOkay.php?ID=<?php echo $ID; ?>">Submit to Audit <i class="fa fa-arrow-right"></i></a>
				 <?php } 
				 }
				 ...
				 ?>
				 <a class="btn btn-danger margin" href="HREncashmentBack.php?ID=<?php echo $ID; ?>"><i class="fa fa-arrow-left"></i> Back to HR</a>
				 <a class="btn btn-success margin" href="AuditEncashmentOkay.php?ID=<?php echo $ID; ?>">Successfully Audit <i class="fa fa-arrow-right"></i></a>
				 <?php 
				 } 
				 }
				 ?>
                  ...
                  <thead class="alig">
                    ...
                    <th rowspan="2">Joining Date</th>
					 ...
					 <th colspan="3" style="text-align:center;font-size:17px;font-family:verdana;">Salary</th>
							   ...
							   <th rowspan="2">Total Encashment Days</th>
							   <th rowspan="2">Encashment Amount</th>
                     ...
                     <tr>
                        ...
                        <th rowspan="">Annual Bonus Allowance</th>
                      <th rowspan="">Gross Salary With Bonus</th>
                      <th rowspan="">Gross Salary Without Bonus</th>
                  ...
                  <tbody>
					...
					?>
                     <tr>
					  ...
					  <th colspan="14"><h4><?php echo $row['Name']; ?></h4></th>
                    </tr>
					 <?php 
					...
					?>
                    <tr>
					  <th colspan="14"><h6><?php echo $row2['Name']; ?><h6></th>
                    </tr>
					<?php 
					$query3 = "SELECT bd.ID AS EncashmentdetailsID,b.EncashmentDate,e.ID,e.AccountNumber,DATE_FORMAT(e.JoiningDate, '%D %b %Y') AS JoiningDate,bd.ID AS PayrollID,e.EmpID,e.Designation,e.CNICNumber,e.CompanyID,e.FirstName,e.LastName,bd.Basic,bd.NetPay,bd.BankorCash,bd.Gross,bd.TotalDays,bd.OtherAllowances,bd.LoanBalance,bd.BonusAmount,bd.OtherDeductions FROM employees e LEFT JOIN encashmentdetails bd ON e.ID = bd.EmpID LEFT JOIN encashment b ON b.ID = bd.EncashmentID WHERE e.ID <> 0 AND e.CompanyID = ".$compid." AND e.Location = ".$row2['ID']." AND bd.EncashmentID = ".$ID." ORDER BY e.ID ASC";
                    ...
                    <th colspan=""><?php echo $c; ?></th>
                      ...
                      <th><?php echo $row3['JoiningDate'];?></th>
                       <th><?php echo $row3['OtherAllowances'];$TTAnnual += $row3['OtherAllowances'];  ?></th>
					  ...
					  <th><?php echo $row3['LoanBalance']; ?></th>
					  <th><?php echo $row3['OtherDeductions'];$TTOtherDeduction += $row3['OtherDeductions']; ?></th>
                     <th><?php echo number_format($row3['NetPay']);$TTNetPay += $row3['NetPay']; ?><!--<br><span class="no-print"><a <?php //echo ($Steps != 0 ? 'disabled' : ''); ?> <?php //echo (($_SESSION['RoleID'] == 'Accounts' OR $_SESSION['RoleID'] == 'Audit') ? 'disabled' : ''); ?> href="EncashmentUpdate.php?ID=<?php //echo $row3['EncashmentdetailsID']; ?>&EncashmentID=<?php //echo $ID; ?>" class="btn btn-info no-print"><i class="fa fa-money no-print"></i></a></span>--></th>
                    </tr>
                     ...
                     <?php
						...
						<tr>
							...
							<th></th>
							<th></th>
							<th><?php echo $TTAnnual; $GTTAnnual = $GTTAnnual + $TTAnnual;  ?></th>
