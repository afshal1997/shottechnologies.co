...
$query="SELECT Step1ID,Step2ID,Step3ID,DATE_FORMAT(Step1Date, '%D %b %Y') AS Step1Date,DATE_FORMAT(Step2Date, '%D %b %Y') AS Step2Date,DATE_FORMAT(Step3Date, '%D %b %Y') AS Step3Date,DATE_FORMAT(Step1Date, '%Y%m') AS Voucher,Steps,MonthEncashment,CompanyID,EncashmentDate,Heading,DATE_FORMAT(DateAdded, '%D %b %Y') AS Created FROM encashment WHERE  ID='" . (int)$ID . "'";
          ...
          <br>
              ...
              <div class="col-lg-12 col-xs-12">
				...
				<div class="row">
					...
					<div class="col-xs-12">
					<b>Reference:</b> &emsp;&emsp;LEAVE ENCASHMENT POSTING
					</div>
					<div class="col-xs-12">
				...
				<table style="font-size:12px;" id="dataTable" class="table table-bordered invoice" style="background-color:white">
                  ...
                  <tbody>
					...
					<?php 
					$query3 = "SELECT SUM(p.NetPay) AS NetPayAmount,SUM(p.NetPay) AS EncashmentAmount FROM encashmentdetails p LEFT JOIN encashment pr ON pr.ID = p.EncashmentID LEFT JOIN employees e ON e.ID = p.EmpID WHERE pr.ID <> 0 AND e.CompanyID = ".$compid." AND pr.ID = ".$ID."";
					// echo $query3;
					// exit();
					...
					?>
					<?php if($row3['EncashmentAmount'] > 0){ ?>
					<tr>
					  <th>STAFF ENCASHMENT</th>
					  <th></th>
					  <th>NO COST CENTRE</th>
					  <th><?php echo number_format($row3['EncashmentAmount'],2); ?></th>
						<th></th>
                    </tr>
					<?php } ?>
					<tr>
