...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			{
			mysql_query("DELETE FROM deductions  WHERE ID IN (" . $BID . ")") or die (mysql_error());
			...
			<b>Delete All selected Deductions.</b>
			</div>';
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Please select Deduction to delete.</b>
			</div>';
		}
	}
?>
...
if(!isset($_SESSION['TitleDeduction']))
$_SESSION['TitleDeduction']="";
...
if(isset($_REQUEST["TitleDeduction"]))
	$_SESSION['TitleDeduction']=trim($_REQUEST["TitleDeduction"]);
...
<script language="javascript">
	...
	function doDelete()
	{
		if($(".chkIds").is(":checked"))
		{
			...
			alert("Please select Deduction to delete");
	}
  ...
  <aside class="right-side">
    ...
    <section class="content">
      <div class="row">
          ...
          <div class="box">
            <div class="box-header">
			  ...
			  <div class="buttons no-print" style="width:70%">
				...
				<button class="btn btn-success margin" type="button" onClick="location.href='AddBulkDeductions.php'">Bulk Upload (CSV)</button>
                <button class="btn btn-success margin" type="button" onClick="location.href='AddNewDeduction.php'">Add New</button>
					...
					<input type="hidden" id="action" name="action" value="" />
                        <div class="col-lg-12 col-xs-12">
							<div class="form-group no-print">
							  <label id="labelimp2" for="Role" >Deduction Filters: </label>
							</div>
                        </div><!-- ./col -->
						...
						<div class="col-lg-3 col-xs-12 no-print">
							<div class="form-group">
							  Title:
							  <select name="TitleDeduction" id="TitleDeduction" class="form-control">
								<option value="">All Titles</option>
								<?php
								 $query = "SELECT DISTINCT Title FROM deductions";
								$res = mysql_query($query);
								while($row = mysql_fetch_array($res))
								{
								echo '<option '.($_SESSION['TitleDeduction'] == $row['Title'] ? 'selected' : '').' value="'.$row['Title'].'">'.$row['Title'].'</option>';
								} 
								?>
								</select>
							</div>
		...
		?>
                    <tr class="noPrint">
                      <td colspan="200" align="center" class="error"><b>No Deduction listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                   <tr>
                     ...
                     <td><?php echo dboutput($row['FirstName']).' '.dboutput($row['LastName']).' ('.dboutput($row['Department']).' - '.dboutput($row['Designation']).')'; ?></td>
					  ...
					  <td align="center" class="noPrint no-print"><button class="btn btn-info margin no-print" type="button" onClick="location.href='EditDeduction.php?ID=<?php echo $row["ID"]; ?>&EmpID=<?php echo $row["EmpID"]; ?>'">Edit</button></td>
                    </tr>
                    <?php
				$i++;
				}
