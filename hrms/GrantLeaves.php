...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			{
				$query = "SELECT ID,EmpID,LeaveType,NumOfDays FROM grant_leaves_quota where ID IN (" . $BID . ")";
				...
				if($num == 1)
				{
					...
					mysql_query("DELETE FROM  grant_leaves_quota  WHERE ID IN (" . $BID . ")") or die (mysql_error());
				}
			...
			<b>Delete All selected Granted Leaves.</b>
			</div>';
			redirect("GrantLeaves.php");
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				FROM grant_leaves_quota d LEFT JOIN employees e ON d.EmpID = e.ID LEFT JOIN employees a ON d.PerformedBy = a.ID WHERE d.ID <>0 ".($Employee != 0 ? ' AND d.EmpID = '.$Employee.'' : '')." ".($FromDate != "" && $TillDate != "" ? ' AND (d.LeaveDate BETWEEN \''.$FromDate.'\' AND \''.$TillDate.'\')' : '')." ORDER BY d.LeaveDate DESC";
				// if($Keywords != "")
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> Grant Leaves <small></small> </h1>
      <ol class="breadcrumb">
        <li><a href="Dashboard.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Grant Leaves</li>
      </ol>
    </section>
    <!-- Main content -->
    <section class="content">
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Grant Leaves</h3>
			  <div class="buttons">
                <?php if($_SESSION['RoleID'] == 'HR' OR $_SESSION['RoleID'] == 'Administrator'){ ?>
                <button class="btn btn-success margin" type="button" onClick="location.href='AddGrantLeave.php'">Add Grant Leave</button>
		...
		?>
                    <tr class="noPrint">
                      <td colspan="11" align="center" class="error"><b>No Grant Leave listed.</b></td>
                    </tr>
                    <?php 
			}
			else
