...
{			
	...
	if($msg=="")
	{
		...
		{
			if(isset($_SESSION["SandwichAdjust"]) && is_array($_SESSION["SandwichAdjust"]))
			{
				foreach($_SESSION["SandwichAdjust"] as $GL)
				{
			...
			<b>Adjust All selected Sandwiches with Special Leaves.</b>
			</div>';
			redirect("SandwitchRoster.php");
		}
		else if($Type == "Auto")
		{
			if(isset($_SESSION["SandwichAdjust"]) && is_array($_SESSION["SandwichAdjust"]))
					...
					if($row['Status'] == "Off Day")
					{
						...
						if($num3 > 0)
						{
							...
							if($row3['CasualLeaves'] > 0.9)
							{
								...
								<i class="fa fa-ban"></i>
								<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
								<b>Adjust All selected Sandwiches.</b>
								</div>';
							}
							...
							{
								$query2="UPDATE roster_login_history SET Status = 'Absent', MStatus='".$Reason."' WHERE ID='" . (int)$GL . "'";
								mysql_query($query2) or die (mysql_error());
...
?>
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> Adjust Sandwiches</h1>
      <ol class="breadcrumb">
        <li><a href="SandwitchRoster.php"><i class="fa fa-dashboard"></i>Sandwitch Roster</a></li>
        <li class="active">Adjust Sandwiches</li>
      </ol>
    </section>
    <!-- Main content -->
    <form role="form" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data" name="frmPage">
      <section class="content">
       <div class="box-footer" style="text-align:right;">
                ...
                <button class="btn btn-primary margin" type="button" onClick="location.href='SandwitchRoster.php'">Cancel</button>
            </div>
              <?php
		  		echo $msg;
