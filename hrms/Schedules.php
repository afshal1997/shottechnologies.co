...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			{
			mysql_query("DELETE FROM schedules  WHERE ID IN (" . $BID . ")") or die (mysql_error());
			...
			<b>Delete All selected Schedules.</b>
			</div>';
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Please select Schedule to delete.</b>
			</div>';
		}
	}
?>
...
<script language="javascript">
	...
	function doDelete()
	{
		if($(".chkIds").is(":checked"))
		{
			...
			alert("Please select Schedule to delete");
	}
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT ID,Status,DayNight,Name AS ScheduleName,ArrivalTime,DepartTime,DATE_FORMAT(DateAdded, '%D %b %Y<br>%r') AS Added,DATE_FORMAT(DateModified, '%D %b %Y<br>%r') AS Updated
				FROM schedules WHERE ID <>0 ";
				if($Keywords != "")
  ...
  <aside class="right-side">
    ...
    <section class="content">
      <div class="row">
          ...
          <div class="box">
            <div class="box-header">
			  ...
			  <div class="buttons">
                ...
                <button class="btn btn-success margin" type="button" onClick="location.href='AddNewSchedule.php'">Add New</button>
		...
		?>
                    <tr class="noPrint">
                      <td colspan="11" align="center" class="error"><b>No Schedule listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                    <tr>
                      ...
                      <td><?php echo dboutput($row['ScheduleName']); ?></td>
					  <td><?php echo dboutput($row["ArrivalTime"]); ?></td>
					  <td><?php echo dboutput($row["DepartTime"]); ?></td>
					  <td><?php if(dboutput($row["DayNight"])==1){echo 'Night';}else{echo 'Day';} ?></td>
					  ...
					  <td align="center" class="noPrint"><button class="btn btn-info margin" type="button" onClick="location.href='EditSchedule.php?ID=<?php echo $row["ID"]; ?>'">Edit</button></td>
                    </tr>
                    <?php				
				}
			} 
