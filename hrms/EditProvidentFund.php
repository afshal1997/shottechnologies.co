...
{	
	...
	if($msg=="")
	{
		$r = mysql_query("SELECT Title FROM provident_funds WHERE 
		Title='".dbinput($Title)."' AND ID <> '".(int)$ID."'") or die(mysql_error());
		if(mysql_num_rows($r) == 0)
		{
			$query="UPDATE provident_funds SET 
			Title = '" . dbinput($Title) . "',
			...
			$msg='<div class="alert alert-success alert-dismissable">
				<i class="fa fa-ban"></i>
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
				<b>ProvidentFund has been Updated.</b>
			</div>';
...
{
	$query="SELECT ID, Approved,ApprovedBy,FrwdEmpID,Title,Amount,Percentage,Date,Type,Description FROM provident_funds WHERE  ID='" . (int)$ID . "'";
	...
	if($num==0)
	{
		$_SESSION["msg"]='<div class="alert alert-danger alert-dismissable">
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Invalid ProvidentFund ID.</b>
		</div>';
		redirect("ProvidentFunds.php");
	}
	else
	{
...
<head>
<meta charset="UTF-8">
<title>Edit Provident Fund</title>
...
?>
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><a href="ProvidentFunds.php"><i class="fa fa-dashboard"></i>Provident Funds</a></li>
        <li class="active">Edit Provident Fund</li>
      </ol>
    </section>
    <!-- Main content -->
    <form role="form" action="<?php echo $_SERVER["PHP_SELF"];?>?ID=<?php echo $ID; ?>"" method="post" enctype="multipart/form-data" name="frmPage">
      ...
      <section class="content">
        <div class="box-footer" style="text-align:right;">
          ...
          <button class="btn btn-primary margin" type="button" onClick="location.href='ProvidentFunds.php'">Cancel</button>
        </div>
        <?php
			  	echo $msg;
