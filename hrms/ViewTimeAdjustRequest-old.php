		...
		else
		{
			...
			redirect("TimeAdjustRequests.php");
				...
				$query3="SELECT lo.ID,li.LoginTime,lo.LogoutTime,li.Status
					 FROM roster_login_history li LEFT JOIN roster_logout_history lo ON li.UserID = lo.UserID AND li.DateAdded = lo.DateAdded WHERE li.ID = ".$RosterID."";
				...
				if($num3 > 0)
				{
					...
					{
						$query2="UPDATE roster_login_history SET MStatus = '".$DisapproveReason."',MLoginTime='".$row['LoginTime']."',LoginTime='". time_format_gracetime($ArrivalTime) ."' WHERE ID='" . (int)$RosterID . "'";
						mysql_query($query2) or die (mysql_error());
						
						$query2="UPDATE roster_logout_history SET MLogoutTime='".$row['LogoutTime']."',LogoutTime='". time_format_gracetime($DepartTime) ."' WHERE ID='" . (int)$row['ID'] . "'";
						mysql_query($query2) or die (mysql_error());
					}
					else
					{
						$query2="UPDATE roster_login_history SET MStatus = '".$DisapproveReason."',MLoginTime='".$row['LoginTime']."',LoginTime='". time_format_gracetime($ArrivalTime) ."', Status = 'Present' WHERE ID='" . (int)$RosterID . "'";
						mysql_query($query2) or die (mysql_error());
				...
				$query5="SELECT e.ID AS EmpID,e.LeavesDays,lo.ID,li.LoginTime,li.DateAdded,lo.LogoutTime,li.Status
					 FROM employees e LEFT JOIN roster_login_history li ON li.UserID = e.ID  LEFT JOIN roster_logout_history lo ON li.UserID = lo.UserID AND li.DateAdded = lo.DateAdded WHERE li.ID = ".$RosterID."";
				...
				if($num5 > 0)
				{
					...
					if($num == 1)
					{
						...
						$query="UPDATE timeadjust_requests SET 
						RosterID = " . (int)$LastID . " WHERE ID = ".$ID."";
						mysql_query($query) or die (mysql_error());
						...
						else
						{
							...
							if (!in_array(''.date("N", $startdate).'', $HalfDays))
							{
								...
								{
									$query3="UPDATE roster_login_history SET MStatus = '".$DisapproveReason."' WHERE UserID='" . (int)$row['ID'] . "' AND DateAdded = '".date("Y-m-d", $startdate)."'";
									mysql_query($query3) or die (mysql_error());
								}
