...
<head>
<meta charset="UTF-8">
<title>Individual Bonuses</title>
<meta content='width=device-width, initial-scale=1, maximum-scale=1, IndividualBonus-scalable=no' name='viewport'>
<!-- bootstrap 3.0.2 -->
<!-- jQuery 2.0.2 -->
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT d.ID,d.EmpID,d.Percentage,a.FirstName AS fn,a.LastName AS ln,a.Department AS dep,a.Designation AS des,DATE_FORMAT(d.Date, '%D %b %Y') AS Added
				FROM individual_bonuses d LEFT JOIN employees a ON d.EmpID = a.EmpID WHERE d.ID <>0 ";
				if($Keywords != "")
					$query .= " AND (d.EmpID LIKE '%" . dbinput($Keywords) . "%')";
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> Individual Bonuses <small></small> </h1>
      <ol class="breadcrumb">
        <li><a href="Dashboard.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Individual Bonuses</li>
      </ol>
    </section>
    <!-- Main content -->
    <section class="content">
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Individual Bonuses</h3>
               <div class="buttons">
                <button class="btn btn-success margin" style="margin-right:auto" type="button" onClick="location.href='AddNewIndividualBonus.php'">Bulk Upload (CSV)</button>
				</button>
		...
		?>
                    <tr class="noPrint">
                      <td colspan="11" align="center" class="error"><b>No Individual Bonus listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                    <tr>
                      ...
                      <td><a href="javascript:getPages(<?php echo $row["ID"]; ?>, 20)" state="1" class="main-page<?php echo $row["ID"]; ?>"><?php echo dboutput($row['EmpID']).' | '.dboutput($row['fn']).' '.dboutput($row['ln']).' ('.dboutput($row['dep']).' - '.dboutput($row['des']).')'; ?></a></td>
					  <td><?php echo dboutput($row["Percentage"]) . '%' ; ?></td>
					  <td><?php echo $row["Added"]; ?></td>
					  <td align="center" class="noPrint"><button class="btn btn-info margin" type="button" onClick="location.href='EditIndividualBonus.php?ID=<?php echo $row["ID"]; ?>'">Edit</button></td>
                    </tr>
                    <?php				
				}
			} 
