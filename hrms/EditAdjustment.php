...
{	
	...
	if($msg=="")
	{
			...
			$msg='<div class="alert alert-success alert-dismissable">
				<i class="fa fa-ban"></i>
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
				<b>Adjustment has been Updated.</b>
			</div>';
...
{
	$query="SELECT ID, Approved,ApprovedBy,EmpID,FrwdEmpID,Title,Amount,Percentage,Date,Type,Description FROM adjustments WHERE  ID='" . (int)$ID . "'";
              ...
              <div class="box-body">
				...
				<div class="form-group">
				  ...
				  <select name="Title" id="Title" class="form-control">
					<option value="" >Select Adjustment Title</option>
					<?php
					 $query = "SELECT ID,Name,Type FROM adjustmenttypes where Status = 1 ORDER BY Name ASC";
					$res = mysql_query($query);
					while($row = mysql_fetch_array($res))
					{
					echo '<option '.($Title == $row['ID'] ? 'selected' : '').' value="'.$row['ID'].'">'.$row['Name'].' | '.($row['Type'] == 1 ? 'Incremental' : 'Decremental').'</option>';
					} 
					?>
				  </select>
                </div>
