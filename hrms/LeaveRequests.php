...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			{
			mysql_query("DELETE FROM leave_approvals  WHERE ID IN (" . $BID . ")") or die (mysql_error());
			...
			<b>Delete All selected Leave Requests.</b>
			</div>';
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Please select Leave Request to delete.</b>
			</div>';
		}
	}
?>
...
<script language="javascript">
	...
	function doDelete()
	{
		if($(".chkIds").is(":checked"))
		{
			...
			alert("Please select Leave Request to delete");
	}
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT l.ID,l.Sender,l.Approval,l.NumOfDays,l.Type,DATE_FORMAT(l.FromDate, '%D %b %Y') AS FromDate,DATE_FORMAT(l.ToDate, '%D %b %Y') AS ToDate,e.EmpID,e.FirstName,e.LastName,e.Department,e.Designation,DATE_FORMAT(l.DateAdded, '%D %b %Y') AS Added
				FROM leave_approvals l LEFT JOIN employees e ON l.Sender = e.ID WHERE l.ID <>0 ";
				if($Keywords != "")
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> Leave Requests <small></small> </h1>
      <ol class="breadcrumb">
    ...
    <section class="content">
      <div class="row">
		...
		?>
                    <tr class="noPrint">
                      <td colspan="11" align="center" class="error"><b>No Leave Request listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                    <tr>
					  ...
					  <td><?php echo dboutput($row["FromDate"]); ?></td>
					  <td><?php echo dboutput($row["ToDate"]); ?></td>
                      <td><?php if(dboutput($row["Approval"])=='1'){echo '<mark style="background-color:#0f0">Approved</mark>';}else if(dboutput($row["Approval"])=='0'){echo '<mark style="background-color:#ff0">Pending</mark>';}else{echo '<mark style="background-color:#f00">DisApproved</mark>';} ?></td>
					  <td><?php echo $row["Added"]; ?></td>
					  <td align="center" class="noPrint"><button  <?php if($row["Approval"] != 0){echo 'disabled';} ?> class="btn btn-info margin" type="button" onClick="location.href='ViewLeaveRequest.php?ID=<?php echo $row["ID"]; ?>'">Details</button></td>
                    </tr>
                    <?php				
				}
			} 
