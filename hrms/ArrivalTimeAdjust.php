...
include("CheckAdminLogin.php");
	...
	if(isset($_SESSION["ArrivalTimeAdjust"]) && is_array($_SESSION["ArrivalTimeAdjust"]))
	{
		$numC = count($_SESSION["ArrivalTimeAdjust"]);
		if($numC == 1)
		{
			if(user_arrivaltime($_SESSION["ArrivalTimeAdjust"][0]) != '')
			{
				$ArrivalTime=user_arrivaltime($_SESSION["ArrivalTimeAdjust"][0]);
			}
			else
			{
				$query="SELECT s.ArrivalTime FROM schedules s LEFT JOIN employees e ON s.ID = e.ScheduleID LEFT JOIN roster_login_history li ON li.UserID = e.ID  LEFT JOIN roster_logout_history lo ON li.UserID = lo.UserID AND li.DateAdded = lo.DateAdded WHERE li.ID = ".$_SESSION["ArrivalTimeAdjust"][0]."";
...
{			
	...
	if($msg=="")
	{
		if(isset($_SESSION["ArrivalTimeAdjust"]) && is_array($_SESSION["ArrivalTimeAdjust"]))
		{
			foreach($_SESSION["ArrivalTimeAdjust"] as $TA)
			{
		...
		<b>Arrival Time Adjust of all selected Attendance.</b>
		</div>';
...
<head>
<meta charset="UTF-8">
<title>Arrival Time Adjust</title>
