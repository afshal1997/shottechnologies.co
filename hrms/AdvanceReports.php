...
<?php
				...
				$query="SELECT e.EmpID,e.FirstName,e.LastName,c.Name AS Company,l.ID,l.Status,l.Amount,l.RemainingAmount,DATE_FORMAT(l.RepaymentDate, '%D %b %Y') AS RepaymentDate FROM advances l LEFT JOIN employees e ON l.EmpID = e.ID LEFT JOIN companies c ON e.CompanyID = c.ID WHERE l.ID <> 0 AND (l.RepaymentDate BETWEEN '".date_format_Ymd($FromDate)."' AND '".date_format_Ymd($TillDate)."') ".($CompanyID != 0 ? ' AND e.CompanyID = '.$CompanyID.'' : '')." ".($Location != 0 ? ' AND e.Location = '.$Location.'' : '')." ".($LoanType != '' ? ' AND l.Status = "'.$LoanType.'"' : '')." ".($Employee != 0 ? ' AND e.ID = '.$Employee.'' : '')." ".($Department != "" ? ' AND e.Department = '.$Department.'' : '')." ".($Designation != "" ? ' AND e.Designation = '.$Designation.'' : '')." ORDER BY ".$SortBy." ASC";
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1>Advance Reports <small></small> </h1>
      <ol class="breadcrumb">
        <li><a href="Dashboard.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Advance Reports</li>
      </ol>
    </section>
    <!-- Main content -->
    <section class="content">
      <div class="row">
					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                        <div class="col-lg-12 col-xs-12">
                           <div class="form-group">
							  <label id="labelimp2" for="Role" >Advance Filters: </label>
							</div>
                        </div><!-- ./col -->
						...
						<div class="col-lg-3 col-xs-12 no-print">
						  ...
						  Advance Status:
							...
							<option value="" >Both Running and Completed</option>
							<option <?php echo ($LoanType == '0' ? 'selected' : ''); ?> value="0">Running</option>
							<option <?php echo ($LoanType == '1' ? 'selected' : ''); ?> value="1">Completed</option>
							</select>
						</div>
						</div><!-- ./col -->
						<div class="col-lg-3 col-xs-12 no-print">
		...
		?>
                    <tr class="noPrint">
                      <td colspan="20" align="center" class="error"><b>No Advance listed.</b></td>
                    </tr>
                    <?php 
			}
			else
			{
				...
				<tr>
				  ...
				  <td><?php echo dboutput(number_format($row["Amount"],2)); $amnt = $amnt + $row["Amount"]; ?></td>
