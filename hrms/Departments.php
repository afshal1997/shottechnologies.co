...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			mysql_query("DELETE FROM departments  WHERE ID IN (" . $ids . ")") or die (mysql_error());
						
			$_SESSION["msg"]='<div class="alert alert-success alert-dismissable">
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
			<b>Delete All selected Departments.</b>
			</div>';
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
			<b>Please select Department to delete.</b>
			</div>';
		}
	}
?>
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT ID,Department,Status, DATE_FORMAT(DateAdded, '%D %b %Y<br>%r') AS Added, 
				DATE_FORMAT(DateModified, '%D %b %Y<br>%r') AS Updated
				FROM departments WHERE ID <>0 ";
				if($Keywords != "")
					$query .= " AND (Department LIKE '%" . dbinput($Keywords) . "%')";
  ...
  <aside class="right-side">
    ...
    <section class="content">
      <div class="row">
          ...
          <div class="box">
            <div class="box-header">
			  ...
			  <div class="buttons">
                ...
                <button class="btn btn-success margin" type="button" onClick="location.href='AddNewDepartment.php'">Add New</button>
		...
		?>
                    <tr class="noPrint">
                      <td colspan="11" align="center" class="error"><b>No Department listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                    <tr>
                      ...
                      <td><a href="javascript:getPages(<?php echo $row["ID"]; ?>, 20)" state="1" class="main-page<?php echo $row["ID"]; ?>"><?php echo dboutput($row["Department"]); ?></a></td>
					  <td><?php if(dboutput($row["Status"])=='1'){echo '<i class="fa fa-fw fa-check-circle"></i>';}else{echo '<i class="fa fa-fw fa-times-circle"></i>';} ?></td>
					  <td><?php echo total_employees_in_department($row["Department"]).' Employees in This Department'; ?></td>
                      <td><?php echo $row["Added"]; ?></td>
                      <td><?php echo $row["Updated"]; ?></td>
					  <td align="center" class="noPrint"><button class="btn btn-info margin" type="button" onClick="location.href='EditDepartment.php?ID=<?php echo $row["ID"]; ?>'">Edit</button></td>
                    </tr>
                    <?php				
				}
			} 
