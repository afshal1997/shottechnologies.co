...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			{
			mysql_query("DELETE FROM externalusers  WHERE ID IN (" . $BID . ")") or die (mysql_error());
			...
			<b>Delete All selected External Accounts.</b>
			</div>';
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Please select External Account to delete.</b>
			</div>';
		}
	}
?>
...
<script language="javascript">
	...
	function doDelete()
	{
		if($(".chkIds").is(":checked"))
		{
			...
			alert("Please select External Account to delete");
	}
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT ID,EmpID,FirstName,LastName,Status,DATE_FORMAT(DateAdded, '%D %b %Y<br>%r') AS Added,DATE_FORMAT(DateModified, '%D %b %Y<br>%r') AS Updated
				FROM externalusers WHERE ID <>0 ";
				if($Keywords != "")
					$query .= " AND (FirstName LIKE '%" . dbinput($Keywords) . "%')";
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> External Accounts <small></small> </h1>
      <ol class="breadcrumb">
        <li><a href="Dashboard.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">External Accounts</li>
      </ol>
    </section>
    <!-- Main content -->
    <section class="content">
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">External Accounts</h3>
			  <div class="buttons">
                <button class="btn btn-success margin" type="button" onClick="location.href='AddNewExternalAccount.php'">Add New</button>
		...
		?>
                    <tr class="noPrint">
                      <td colspan="11" align="center" class="error"><b>No External Account listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                    <tr>
					  ...
					  <td align="center" class="noPrint"><button class="btn btn-info margin" type="button" onClick="location.href='EditExternalAccount.php?ID=<?php echo $row["ID"]; ?>'">Edit</button></td>
                    </tr>
                    <?php				
				}
			} 
