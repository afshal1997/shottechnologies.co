...
{	
	...
	if($msg=="")
	{
		$r = mysql_query("SELECT EmpID FROM basicsalary WHERE 
		EmpID='".dbinput($EmpID)."' AND ID <> '".(int)$ID."'") or die(mysql_error());
		if(mysql_num_rows($r) == 0)
		{
			$query="UPDATE basicsalary SET 
			...
			$msg='<div class="alert alert-success alert-dismissable">
				<i class="fa fa-ban"></i>
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
				<b>Basic Salary has been Updated.</b>
			</div>';
...
{
	$query="SELECT ID, Approved,ApprovedBy,EmpID,FrwdEmpID,Amount,Date FROM basicsalary WHERE  ID='" . (int)$ID . "'";
            ...
            <div style="padding:15px;" class="box-primary">
              ...
              <div class="box-body">
				...
				<div class="form-group">
                  <label id="labelimp" class="labelimp" for="AmountPercentage">Amount: </label>
                  <?php 
				echo '<input type="text" maxlength="100" id="AmountPercentage" name="AmountPercentage" placeholder="'.CURRENCY_SYMBOL.'" class="form-control"  value="'.$Amount.'" />';
				?>
                </div>
				...
				<div class="form-group">
				  ...
				  <select name="EmpID" id="EmpID" class="form-control">
					...
					{
					echo '<option '.($EmpID == $row['ID'] ? 'selected' : '').' value="'.$row['ID'].'">'.$row['FirstName'].' '.$row['LastName'].' ('.$row['Department'].' - '.$row['Designation'].')</option>';
					} 
					?>
				  </select>
                </div>
