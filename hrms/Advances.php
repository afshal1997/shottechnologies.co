...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			{
			mysql_query("DELETE FROM advances  WHERE Status = 0 AND ID IN (" . $BID . ")") or die (mysql_error());
			...
			<b>Delete All selected Running Advances.</b>
			</div>';
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Please select Advance to delete.</b>
			</div>';
		}
	}
?>
...
<script language="javascript">
	...
	function doDelete()
	{
		if($(".chkIds").is(":checked"))
		{
			...
			alert("Please select Advance to delete");
	}
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT d.ID,d.Code,d.Status,d.Reason,d.Amount,DATE_FORMAT(d.RepaymentDate, '%D %b %Y') AS RepaymentDate,e.EmpID,e.FirstName,e.LastName,e.Department,e.Designation,a.FirstName AS fn,a.LastName AS ln,a.Department AS dep,a.Designation AS des,DATE_FORMAT(d.DateAdded, '%D %b %Y') AS Added,DATE_FORMAT(d.DateModified, '%D %b %Y') AS Updated
				FROM advances d LEFT JOIN employees e ON d.EmpID = e.ID LEFT JOIN employees a ON d.ApprovedBy = a.ID WHERE d.ID <>0 ".($Employee != 0 ? ' AND d.EmpID = '.$Employee.'' : '')." ".($CompanyID != '' ? ' AND FIND_IN_SET(e.CompanyID,\''.$CompanyID.'\')' : '')." ".($Status != 10 ? ' AND d.Status = '.$Status.'' : '')." ".($FromDate != "" && $TillDate != "" ? ' AND (d.RepaymentDate BETWEEN \''.$FromDate.'\' AND \''.$TillDate.'\')' : '')." ";
				if($Keywords != "")
					$query .= " AND (d.Code LIKE '%" . dbinput($Keywords) . "%')";
				$query .= " ORDER BY d.RepaymentDate DESC";
  ...
  <aside class="right-side">
    ...
    <section class="content">
      <div class="row">
          ...
          <div class="box">
            <div class="box-header">
			  ...
			  <div class="buttons">
               ...
               <button class="btn btn-success margin" type="button" onClick="location.href='AddManualAdvance.php'">Add Manual Advance</button>
		...
		?>
                    <tr class="noPrint">
                      <td colspan="11" align="center" class="error"><b>No Advance listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                    <tr>
					  ...
					  <td align="center" class="noPrint"><button class="btn btn-info margin" type="button" onClick="location.href='AdvanceSlip.php?ID=<?php echo $row["ID"]; ?>'">Slip</button></td>
					  <td align="center" class="noPrint"><button class="btn btn-info margin" type="button" onClick="location.href='EditAdvance.php?ID=<?php echo $row["ID"]; ?>'">Details</button></td>
                    </tr>
                    <?php				
				}
			} 
