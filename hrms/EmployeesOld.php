...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT ID,Status,Photo,EmpID,FirstName,LastName,Role,Department,Designation,DATE_FORMAT(DateAdded, '%D %b %Y<br>%r') AS Added, 
				DATE_FORMAT(DateModified, '%D %b %Y<br>%r') AS Updated
				FROM employees WHERE ID <>0 ";
				if($Keywords != "")
					$query .= " AND (EmpID LIKE '%" . dbinput($Keywords) . "%')";
