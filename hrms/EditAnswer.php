...
{	
	...
	if($msg=="")
	{
			$query="UPDATE answers SET QuestionID = '".(int)$QuestionID."', Answer = '".dbinput($Answer)."', Status = '".$Status."', DateModified = NOW() WHERE ID='".(int)$ID."'";
...
{
	$query="SELECT ID, QuestionID, Answer, Status FROM answers WHERE  ID='" . (int)$ID . "'";
            ...
            <div style="padding:15px;" class="box-primary">
              ...
              <div class="box-body">
                <div class="form-group">
                  <label id="labelimp" for="QuestionID"> Question: </label>
				  <select name="QuestionID" id="QuestionID" class="form-control">
					<option value="0">Select QuestionID</option>
					<?php
						$query1 = "SELECT ID, Question FROM questions where Status = 1 ORDER BY ID DESC";
						$data = mysql_query($query1);
						while($row = mysql_fetch_array($data))
						{
							echo '<option '.($QuestionID == $row['ID'] ? ' selected ' : '').' value="'.$row['ID'].'">'.$row['Question'].'</option>';
						}
					?>
				  </select>
                </div>
