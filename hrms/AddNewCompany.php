...
if($_SESSION['RoleID'] == 'Administrator' OR user_authentication($_SESSION['UserID'],'SecureInsCopmany') OR external_user_authentication($_SESSION['UserID'],'SecureInsCopmany'))
{}else{redirect("Companies.php");}
	...
	$PerDayHours=0;
	$DeductionOnLates="";
	$DeductionOnLatesTypes="Included";
	$DeductionOnLatesAdjustment="No";
	$RefreshQuota="";
	$CurrencySymbol="";
	$PassingPercentage=0;
	$NumOfAttempts=0;
	...
	$EHalfDays31=0;
				  ...
				  // for($i = 1; $i <= 31; $i++)
				  // {
					// echo "\$LateDays".$i."=\$row[\"LateDays".$i."\"];<br>
					// \$EarlyDays".$i."=\$row[\"EarlyDays".$i."\"];<br>
					// \$EDays".$i."=\$row[\"EDays".$i."\"];<br>
					// \$EHalfDays".$i."=\$row[\"EHalfDays".$i."\"];<br>";
				  // }
	// exit();
...
{	
	...
	if(isset($_FILES["flPage"]) && $_FILES["flPage"]['name'] != "")
	{
		...
		else if($_FILES["flPage"]['size'] > (MAX_IMAGE_SIZE*1024))
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			Logo size must be ' . (MAX_IMAGE_SIZE/1024) . ' MB or less.
			</div>';
		}
	}
	...
	if($msg=="")
	{
			$query="INSERT INTO companies SET DateAdded=NOW(),
				...
				BonusAmntPer13 = '" . (int)$BonusAmntPer13 . "'";
			...
			$_SESSION["msg"]='<div class="alert alert-success alert-dismissable">
				<i class="fa fa-ban"></i>
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
				<b>Company has been Added.</b>
			</div>';
			
			if(isset($_FILES["flPage"]) && $_FILES["flPage"]['name'] != "")
			{
				if(is_file(DIR_COMPANY_LOGO . $StoreImage))
					unlink(DIR_COMPANY_LOGO . $StoreImage);
            ...
            <div style="padding:15px;" class="box-primary">
              ...
              <div class="box-body">
				...
				<div class="form-group">
                  <label id="labelimp" for="logo" class="labelimp" >Logo: </label>
                  <input type="file" id="logo" name="flPage" />
                </div>
