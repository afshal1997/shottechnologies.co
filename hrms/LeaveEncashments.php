...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			$queryCheck="SELECT ID FROM encashment WHERE ID IN (" . $BID . ") AND Steps = 0";
			$resultCheck = mysql_query ($queryCheck) or die(mysql_error()); 
			$numCheck = mysql_num_rows($resultCheck);
			if($numCheck == 1)
			{
			
			mysql_query("DELETE FROM encashment  WHERE ID IN (" . $BID . ")") or die (mysql_error());
			mysql_query("DELETE FROM encashmentdetails WHERE EncashmentID IN (" . $BID . ")") or die (mysql_error());
			...
			<b>Delete All selected Leave Encashments.</b>
			</div>';
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Please select Leave Encashment to delete.</b>
			</div>';
		}
	}
?>
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				FROM encashment p LEFT JOIN employees e ON p.PerformedBy = e.ID WHERE p.ID <>0 ".($_SESSION['PayrollMonthAuditReport'] != "" ? ' AND p.MonthEncashment = \''.$_SESSION['PayrollMonthAuditReport'].'\'' : '')." ".($_SESSION['PayrollCompanyIDAuditReport'] != "" ? ' AND FIND_IN_SET(p.CompanyID,\''.$_SESSION['PayrollCompanyIDAuditReport'].'\')' : '')." ORDER BY p.ID DESC";
          ...
          <div class="box">
            <div class="box-header">
			  ...
			  <?php if($_SESSION['RoleID'] == 'HR' OR $_SESSION['RoleID'] == 'Administrator'){ ?>
                <button class="btn btn-success margin" type="button" onClick="location.href='AddNewLeaveEncashment.php'">Add New</button>
					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                        <div class="col-lg-3 col-xs-12">
						   ...
						   <div class="form-group">
							  <label id="labelimp2" for="Role" >Encashments Filters: </label>
							</div>
                        </div><!-- ./col -->
						<div class="col-lg-3 col-xs-6">
							<div class="form-group">
							  ...
							  <select name="PayrollMonth" id="PayrollMonth" class="form-control">
								<option value="">Select Encashment Month</option>
								<?php
								 $query = "SELECT DISTINCT MonthEncashment FROM encashment where ID <> 0 ORDER BY ID DESC";
								$res = mysql_query($query);
								while($row = mysql_fetch_array($res))
								{
								echo '<option '.($_SESSION['PayrollMonthAuditReport'] == $row['MonthEncashment'] ? 'selected' : '').' value="'.$row['MonthEncashment'].'">'.$row['MonthEncashment'].'</option>';
								} 
								?>
							  </select>
							</div>
