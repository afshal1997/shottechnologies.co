...
{	
	...
	if($msg=="")
	{
		$r = mysql_query("SELECT Date FROM gazetted_holidays WHERE 
		Date='".dbinput($Date)."' AND ID <> '".(int)$ID."'") or die(mysql_error());
		if(mysql_num_rows($r) == 0)
		{
			
			$query="UPDATE gazetted_holidays SET 
			Title = '" . dbinput($Title) . "',
			Date = '" . dbinput($Date) . "',
			...
			$msg='<div class="alert alert-success alert-dismissable">
				<i class="fa fa-ban"></i>
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
				<b>Gazetted Holiday has been Updated.</b>
			</div>';
...
{
	$query="SELECT ID, Approved,ApprovedBy,FrwdEmpID,Title,Date FROM gazetted_holidays WHERE  ID='" . (int)$ID . "'";
	...
	if($num==0)
	{
		$_SESSION["msg"]='<div class="alert alert-danger alert-dismissable">
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Invalid Gazetted Holiday ID.</b>
		</div>';
...
<head>
<meta charset="UTF-8">
<title>Edit Gazetted Holiday</title>
