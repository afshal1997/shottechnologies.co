...
$query="SELECT Step1ID,Step2ID,Step3ID,DATE_FORMAT(Step1Date, '%D %b %Y %r') AS Step1Date,DATE_FORMAT(Step2Date, '%D %b %Y %r') AS Step2Date,DATE_FORMAT(Step3Date, '%D %b %Y %r') AS Step3Date,Steps,MonthPayroll,CompanyID,FromDate,ToDate,NumOfDays,DATE_FORMAT(DateAdded, '%D %b %Y') AS Created FROM payroll WHERE  ID='" . (int)$ID . "'";
...
$(window).load(function() {
	...
	$("thead.table-fixed tr th").each(function (i){
       $(this).width($($("thead.table-alig tr:first th")[i]).width());
})});
</script>
<style>
#labelimp {
			...
			?>
              ...
              <div id="tblExport">
                ...
                <table style="font-size:8px;" id="dataTable" class="table table-bordered invoice" style="background-color:white">
                  ...
                  <thead class="align table-alig table-fixed">
                    <tr>
                      <th rowspan="2">S#</th>
					  <th rowspan="2">Emp ID</th>
                      <th rowspan="2">Employee Name</th>
                  ...
                  <tbody>
					...
					<?php 
					$query3 = "SELECT pr.FromDate,pr.ToDate,e.ID,p.ID AS PayrollID,e.EmpID,e.Designation,e.CNICNumber,p.CompanyID,e.FirstName,e.LastName,p.Basic,p.LEDeductions,p.NetPay,p.GrossOfDays,p.AllowanceBreakup,p.BankorCash,p.Gross,p.Present,p.Lates,p.Earlies,p.HalfDays,p.OffDays,p.Leaves,p.Absent,p.TotalDays,p.WOvertimeH,p.WOvertimeA,p.LOvertimeH,p.LOvertimeA,p.OvertimeHolidayDays,p.OtherAllowances,p.OtherDeductions,p.IncomeTax FROM employees e LEFT JOIN payrolldetails p ON e.ID = p.EmpID LEFT JOIN payroll pr ON pr.ID = p.PayID WHERE e.ID <> 0 AND p.CompanyID = ".$compid." AND p.Location = ".$row2['ID']." AND p.PayID = ".$ID." ORDER BY e.ID ASC";
					...
					if($num3 > 0)
					{
						...
						{
					?>
					<tr <?php echo ($row3['TotalDays'] > $num_of_days ?  'style="background-color:red;color:white;"' : '' ); ?> <?php echo ($row3['NetPay'] < 0 ?  'style="background-color:blue;color:white;"' : '' ); ?>>
                    <th colspan=""><?php echo $c; ?></th>
					  ...
					  <th><?php echo $row3['FirstName'].' '.$row3['LastName']; ?><br><?php echo $row3['Designation']; ?><br><br><?php echo $row3['CNICNumber']; ?></th>
                       <th><?php echo number_format($row3['Basic'],2);$TTBasic += $row3['Basic'];  ?></th>
                      <th><?php echo number_format($row3['AllowanceBreakup'],2);$TTAllowance += $row3['AllowanceBreakup']; ?></th>
					  <th><?php echo number_format($row3['Gross'],2);$TTGross += $row3['Gross']; ?></th>
                     ...
                     <th><?php echo $row3['GrossOfDays'];$TTDaysGross += $row3['GrossOfDays']; ?></th>
                      ...
                      <th>
					   W H: <?php echo number_format($row3['WOvertimeA'],2);?>
					   <br>
					   H D: <?php echo number_format($row3['LOvertimeA'],2);?>
					   <br>____________
					   Total: <?php echo number_format($row3['WOvertimeA'] + $row3['LOvertimeA'],2);$TTOTA += round($row3['WOvertimeA'] + $row3['LOvertimeA']); ?>
					  </th>
					  <th><?php echo number_format($row3['OtherAllowances'],2);$TTOtherAllowance += $row3['OtherAllowances']; ?></th>
                      <th><?php echo number_format($row3['OtherAllowances'] + round($row3['WOvertimeA'] + $row3['LOvertimeA']) + $row3['GrossOfDays'],2);$TTGrossPay += $row3['OtherAllowances'] + round($row3['WOvertimeA'] + $row3['LOvertimeA']) + $row3['GrossOfDays']; ?></th>
                    <th><?php echo number_format($row3['OtherDeductions'],2);$TTOtherDeduction += $row3['OtherDeductions']; ?></th>
                      <th><?php echo number_format($row3['IncomeTax'],2);$TTIncomeTax += $row3['IncomeTax']; ?></th>
                     <th><?php echo number_format($row3['NetPay'],2);$TTNetPay += $row3['NetPay']; ?></th>
						...
						<div class="modal-dialog">
							<div class="modal-content">
								<div class="modal-header">
									...
									<h4 class="modal-title"><i class="fa fa-money"></i> Allowances and Deductions Detail | <?php echo $row3['FirstName'].' '.$row3['LastName'].' ('.$row3['EmpID'].')'; ?></h4>
								</div>
