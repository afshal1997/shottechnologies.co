...
{	
	...
	if($msg=="")
	{
		...
		SubDepartment='".dbinput($SubDepartment)."' AND DepartmentID='".$DepartmentID . "' AND ID <> '".(int)$ID."'") or die(mysql_error());
		if(mysql_num_rows($r) == 0)
		{
			$query="UPDATE subdepartments SET Status='".(int)$Status . "', DateModified=NOW(),
			...
			$msg='<div class="alert alert-success alert-dismissable">
				<i class="fa fa-ban"></i>
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
				<b>SubDepartment has been Updated.</b>
			</div>';
...
{
	$query="SELECT ID, SubDepartment,DepartmentID,Status,DATE_FORMAT(DateAdded, '%D %b %Y %r') AS Added, DATE_FORMAT(DateModified,
	 '%D %b %Y %r') AS Updated FROM subdepartments WHERE  ID='" . (int)$ID . "'";
	...
	if($num==0)
	{
		$_SESSION["msg"]='<div class="alert alert-danger alert-dismissable">
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Invalid SubDepartment ID.</b>
		</div>';
		redirect("SubDepartments.php");
	}
	else
	{
...
<title>Edit SubDepartment</title>
