...
{	
	...
	if($msg=="")
	{
		$r = mysql_query("SELECT EmpID FROM individual_bonuses WHERE 
		EmpID='".dbinput($EmpID)."' AND ID <> '".(int)$ID."'") or die(mysql_error());
		if(mysql_num_rows($r) == 0)
		{
			$query="UPDATE individual_bonuses SET 
			EmpID = '" . dbinput($EmpID) . "',
			...
			$msg='<div class="alert alert-success alert-dismissable">
				<i class="fa fa-ban"></i>
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
				<b>Individual Bonus has been Updated.</b>
			</div>';
...
{
	$query="SELECT ID, EmpID,Percentage,Date FROM individual_bonuses WHERE  ID='" . (int)$ID . "'";
	...
	if($num==0)
	{
		$_SESSION["msg"]='<div class="alert alert-danger alert-dismissable">
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Invalid Individual Bonus ID.</b>
		</div>';
		redirect("IndividualBonuses.php");
	}
	else
	{
...
<head>
<meta charset="UTF-8">
<title>Edit Individual Bonus</title>
...
?>
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1>Edit Individual Bonus</h1>
      <ol class="breadcrumb">
        <li><a href="IndividualBonuses.php"><i class="fa fa-dashboard"></i>Individual Bonuss</a></li>
        <li class="active">Edit Individual Bonus</li>
      </ol>
    </section>
    <!-- Main content -->
    <form role="form" action="<?php echo $_SERVER["PHP_SELF"];?>?ID=<?php echo $ID; ?>" method="post" enctype="multipart/form-data" name="frmPage">
      ...
      <section class="content">
        <div class="box-footer" style="text-align:right;">
          ...
          <button class="btn btn-primary margin" type="button" onClick="location.href='IndividualBonuses.php'">Cancel</button>
        </div>
        <?php
			  	echo $msg;
            ...
            <div style="padding:15px;" class="box-primary">
              ...
              <div class="box-body">
				...
				<div class="form-group">
                  <label id="labelimp" for="EmpID" >Employee: </label>
				  <select disabled id="EmpID" class="form-control">
					...
					{
					echo '<option '.($EmpID == $row['EmpID'] ? 'selected' : '').' value="'.$row['EmpID'].'">'.$row['EmpID'].' | '.$row['FirstName'].' '.$row['LastName'].' ('.$row['Department'].' - '.$row['Designation'].')</option>';
					} 
					?>
					</select>
                </div>
