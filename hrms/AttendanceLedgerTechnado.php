...
if(!isset($_SESSION['SubDepartmentLedger']))
$_SESSION['SubDepartmentLedger']="";

if(!isset($_SESSION['BusinessUnitLedger']))
$_SESSION['BusinessUnitLedger']="";
...
if(isset($_REQUEST["SubDepartment"]))
		$_SESSION['SubDepartmentLedger']=trim($_REQUEST["SubDepartment"]);
if(isset($_REQUEST["BusinessUnit"]))
		$_SESSION['BusinessUnitLedger']=trim($_REQUEST["BusinessUnit"]);
...
<body class="skin-blue attendance-pg-wrap" style="font-size:12px;">
<!--<div id="bottomMenu">
  ...
  <aside class="right-side">
          ...
          <br>
			...
			<div class="row margin">
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
						...
						<div class="col-lg-2 col-xs-12 no-print">
						  ...
						  Business Unit:
							...
							while($row = mysql_fetch_array($res))
							{
							echo '<option '.($_SESSION['BusinessUnitLedger'] == $row['ID'] ? 'selected' : '').' value="'.$row['ID'].'">'.$row['BusinessUnit'].'</option>';
							} 
							?>
							</select>
						</div>
						...
						<div class="col-lg-2 col-xs-12 no-print">
						  ...
						  Sub Department:
							...
							while($row = mysql_fetch_array($res))
							{
							echo '<option '.($_SESSION['SubDepartmentLedger'] == $row['ID'] ? 'selected' : '').' value="'.$row['ID'].'">'.$row['SubDepartment'].' ('.$row['Department'].')</option>';
							} 
							?>
							</select>
						</div>
					...
					<span class='new-span-for-icon'>	<i style="width:10px;height:10px;border:1px black solid;background-color:lightblue">&emsp;&nbsp;</i> Half Day </span>
						...
						<div class="col-lg-3 col-xs-12 no-print">
                           <div class="form-group">
						   <input type="checkbox" <?php echo ($_SESSION['LateLedger'] == 1 ? ' checked="checked"' : ''); ?> value="1" id="LateLedger" name="LateLedger"> <label for="LateLedger">Lates</label>&emsp;&emsp;
						   <input type="checkbox" <?php echo ($_SESSION['EarlyDepLedger'] == 1 ? ' checked="checked"' : ''); ?> value="1" id="EarlyDepLedger" name="EarlyDepLedger"> <label for="EarlyDepLedger">Earlies</label>&emsp;&emsp;&emsp;
						   <input type="checkbox" <?php echo ($_SESSION['HalfDayLedger'] == 1 ? ' checked="checked"' : ''); ?> value="1" id="HalfDayLedger" name="HalfDayLedger"> <label for="HalfDayLedger">Halfdays</label><br>
							...
							if($row["OvertimePolicy"] != 0)
							{
							  ...
							  else
							  {
							   list($hours, $minutes) = explode(':', $row["TArrivalTime"]);
									...
									$TotalOvertimeHoursHolidays = $TotalOvertimeHoursHolidays + $hours;
									$TotalOvertimeMinutesHolidays = $TotalOvertimeMinutesHolidays + $minutes;
							  }
							}
						}
						else
	...
	<div id="printable">
		 <table class="blue table table-bordered" style="border:none">
                  <thead class="">
					<tr class="no-print">
					<th class="no-print" colspan="20" style="text-align:right;background-color:white;width:1130px;">
					<a class="btn btn-primary margin no-print" onclick="window.print();"><i class="fa fa-print"></i> Print</a>
					</th>
					</tr>
                    <tr style="color:white;" class="head">
					  ...
					  <th style="width:70px">Mod Time In</th>
                      <th style="width:70px">Mod Time Out</th>
					  <th style="width:85px">Schedule</th>
					  <th style="width:70px">Late Arr</th>
					  <th style="width:70px">Early Dep</th>
					  <th style="width:70px">Working Hours</th>
					  <th style="width:70px">Overtime</th>
					  <th style="width:90px">Modified Remarks</th>
		...
		?>
                    <tr class="noPrint">
                      <td colspan="20" align="center" class="error" style="width:1130px"><b>No Attandence listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                    <tr <?php echo ($row["Status"] == 'Absent' ? ($Legends == 1 ? 'style="background-color:red;color:white;"' : '') : '' ); ?><?php echo ($row["Status"] == 'Off Day' ? ($Legends == 1 ? 'style="background-color:pink;"' : '') : '' ); ?><?php echo ($row["Status"] == 'Leave' ? ($Legends == 1 ? 'style="background-color:yellow;"' : '') : '' ); ?><?php echo ($row["HalfDay"] == 1 ? ($Legends == 1 ? 'style="background-color:lightblue;color:black"' : '') : '' ); ?><?php echo ($row["Late"] == 1 || $row["EarlyDep"] == 1 ? ($Legends == 1 ? 'style="background-color:lightgreen;color:black"' : '') : '' ); ?>>
					  ...
					  <!--<td><?php //echo dboutput($row['Designation']); ?></td>
					  <td><?php //echo dboutput($row['Department']); ?></td>-->
					  <td style="width:100px;"><?php echo dboutput($row["Day"])."<br>".dboutput($row["Date"]); ?></td>
					  <td <?php echo (dboutput($row["MArrivalTime"]) != NULL ? ($Legends == 1 ? 'style="background-color:#c1c1c1;width:70px"' : 'style="width:70px;"') : 'style="width:70px;"'); ?>><?php echo (dboutput($row["ArrivalTime"]) == NULL ? '' : dboutput($row["ArrivalTime"])); ?></td>
					  <td <?php echo (dboutput($row["MDepartTime"]) != NULL ? ($Legends == 1 ? 'style="background-color:#c1c1c1;width:70px"' : 'style="width:70px;"') : 'style="width:70px;"'); ?>><?php echo (dboutput($row["DepartTime"]) == NULL ? '' : dboutput($row["DepartTime"])); ?></td>
					  <td style="width:70px;"><?php echo (dboutput($row["MArrivalTime"]) == '12:00 AM' ? '' : dboutput($row["MArrivalTime"])); ?></td>
					  <td style="width:70px;"><?php echo (dboutput($row["MDepartTime"]) == '12:00 AM' ? '' : dboutput($row["MDepartTime"])); ?></td>
					  <td style="width:85px;"><?php echo revert_time_format_gracetime($row["ScheduleArrivalTime"]).' To<br>'.revert_time_format_gracetime($row["ScheduleDepartTime"]); ?></td>
					  <td style="width:70px;">
					  <?php 
					  if(dboutput($row["Late"]) == 1)
						{
					  ...
					  <td style="width:90px;"><?php echo dboutput($row["MStatus"]); ?></td>
					  <td style="width:70px;"><?php echo ($row["HalfDay"] == 1 ? 'HalfDay<br>' : '' ); ?><?php echo ($row["Late"] == 1 ? 'Late<br>' : '' ); ?><?php echo ($row["EarlyDep"] == 1 ? 'Early<br>' : '' ); ?><?php echo dboutput($row["Status"]); ?></td>
                    </tr>
                    <?php	
					$i++;
					}
		...
		?>
				<?php
				  ...
				  ?>
					<tr>
					  <td style="color:white;background-color:black;width:1130px;" colspan="20">Total Days: <?php echo $TotalDays; ?>, Present: <?php echo $TotalPresent; ?>, Absent <?php echo $TotalAbsent; ?>, OffDays <?php echo $TotalOffDays; ?>, Leaves <?php echo $TotalLeaves; ?>, Halfdays <?php echo $TotalHalfdays; ?>, Lates <?php echo $TotalLates; ?>, EarlyDepart <?php echo $TotalEarlyDepart; ?>, Total Hours <?php echo $TotalHours; ?>, Working Hours <?php echo $TotalWorkingHours; ?>, DifferenceWorkingHours <?php echo $TotalHours - $TotalWorkingHours; ?>, Working OT Hours <?php echo $TotalOvertimeHoursWorking; ?>, Holidays OT Hours <?php echo $TotalOvertimeHoursHolidays; ?></td>
                    </tr>
				<?php 
					}
				?>
