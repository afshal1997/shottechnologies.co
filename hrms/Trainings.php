...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			mysql_query("DELETE FROM trainings  WHERE ID IN (" . $BID . ")") or die (mysql_error());
			...
			<b>Delete All selected Trainings.</b>
			</div>';
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Please select Training to delete.</b>
			</div>';
		}
	}
?>
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT tr.ID,tr.Title,ts.TestName,tr.StartDate,tr.EndDate,tr.StartTime,tr.EndTime,tr.Color,DATE_FORMAT(tr.DateAdded, '%D %b %Y<br>%r') AS Added, 
				DATE_FORMAT(tr.DateModified, '%D %b %Y<br>%r') AS Updated
				FROM trainings tr LEFT JOIN tests ts ON tr.Test = ts.ID WHERE tr.ID <>0 ";
				if($Keywords != "")
      ...
      <div class="row">
          ...
          <div class="box">
            <div class="box-header">
			  ...
			  <div class="buttons" style="width:50%">
                <button class="btn btn-success margin" type="button" onClick="location.href='AddNewTraining.php'">Add New</button>
		...
		?>
                    <tr>
					  ...
					  <td align="center" class="noPrint"><button class="btn btn-info margin" type="button" onClick="location.href='EditTraining.php?ID=<?php echo $row["ID"]; ?>'">Edit</button></td>
					  <td align="center" class="noPrint"><button class="btn btn-primary margin" type="button" onClick="location.href='TrainingsDetails.php?ID=<?php echo $row["ID"]; ?>'">Details</button></td>
                    </tr>
                    <?php				
				}
			} 
