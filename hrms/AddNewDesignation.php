...
{	
	...
	if(isset($_POST["Status"]) && ((int)$_POST["Status"] == 0 || (int)$_POST["Status"] == 1))
		...
		Designation='".dbinput($Designation)."'") or die(mysql_error());
		if(mysql_num_rows($r) != 0)
		{
			$msg='<div class="alert alert-danger alert-dismissable">
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Designation is Duplicated.</b>
			</div>';
		}
	...
	if($msg=="")
	{
		...
		$query="INSERT INTO designations SET Status='".(int)$Status . "', DateAdded=NOW(),
		...
		<b>Designation has been added.</b>
		</div>';		

		redirect("AddNewDesignation.php");	
	}
