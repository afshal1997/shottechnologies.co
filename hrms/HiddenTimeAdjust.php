...
include("CheckAdminLogin.php");
	...
	if(isset($_SESSION["TimeAdjust"]) && is_array($_SESSION["TimeAdjust"]))
	{
		...
		if($numC == 1)
		{
			...
			{
				$query="SELECT s.ArrivalTime FROM schedules s LEFT JOIN employees e ON s.ID = e.ScheduleID LEFT JOIN roster_login_history li ON li.UserID = e.ID  LEFT JOIN roster_logout_history lo ON li.UserID = lo.UserID AND li.DateAdded = lo.DateAdded WHERE li.ID = ".$_SESSION["TimeAdjust"][0]."";
	...
	if(isset($_SESSION["TimeAdjust"]) && is_array($_SESSION["TimeAdjust"]))
	{
		...
		if($numC == 1)
		{
			...
			{
				$query="SELECT s.DepartTime FROM schedules s LEFT JOIN employees e ON s.ID = e.ScheduleID LEFT JOIN roster_login_history li ON li.UserID = e.ID  LEFT JOIN roster_logout_history lo ON li.UserID = lo.UserID AND li.DateAdded = lo.DateAdded WHERE li.ID = ".$_SESSION["TimeAdjust"][0]."";
				$result = mysql_query($query) or die (mysql_error());
				$num = mysql_num_rows($result);
				if($num == 1)
				{
		...
		if(isset($_SESSION["TimeAdjust"]) && is_array($_SESSION["TimeAdjust"]))
		{
			foreach($_SESSION["TimeAdjust"] as $TA)
			{
				...
				if($num5 > 0)
				{
					...
					{
						$query2="UPDATE roster_login_history SET LoginTime='". time_format_gracetime($ArrivalTime) ."', HalfDay = 0,Late = 0,EarlyDep = 0, MStatus=CONCAT(IFNULL(MStatus,''), ' <br>".$Reason."') WHERE ID='" . (int)$TA . "'";
						mysql_query($query2) or die (mysql_error());
						
						$query2="UPDATE roster_logout_history SET LogoutTime='". time_format_gracetime($DepartTime) ."' WHERE ID='" . (int)$row5['ID'] . "'";
						mysql_query($query2) or die (mysql_error());
					}
					else
					{
						$query2="UPDATE roster_login_history SET LoginTime='". time_format_gracetime($ArrivalTime) ."', Status = 'Present',HalfDay = 0,Late = 0,EarlyDep = 0, MStatus=CONCAT(IFNULL(MStatus,''), ' <br>".$Reason."') WHERE ID='" . (int)$TA . "'";
						mysql_query($query2) or die (mysql_error());
					...
					if($num == 1)
					{
						...
						$query="INSERT INTO temp_login_history (UserID, LoginTime, MLoginTime, LoginDate, Status, MStatus, DateAdded) SELECT UserID, LoginTime, MLoginTime, LoginDate, Status, MStatus, DateAdded FROM roster_login_history WHERE UserID='" . (int)$row['ID'] . "' AND DateAdded = '".date("Y-m-d", $startdate)."' AND Status <> 'Absent'";
						...
						$query="INSERT INTO temp_logout_history (UserID, LogoutTime, MLogoutTime, LogoutDate, DateAdded)
							...
							WHERE lo.UserID='" . (int)$row['ID'] . "' AND lo.DateAdded = '".date("Y-m-d", $startdate)."' AND li.Status <> 'Absent'";
						mysql_query ($query) or die(mysql_error());
