...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			mysql_query("DELETE FROM designations  WHERE ID IN (" . $ids . ")") or die (mysql_error());
						
			$_SESSION["msg"]='<div class="alert alert-success alert-dismissable">
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
			<b>Delete All selected Designations.</b>
			</div>';
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
			<b>Please select Designation to delete.</b>
			</div>';
		}
	}
?>
...
<script language="javascript">
	...
	function doDelete()
	{
		if($(".chkIds").is(":checked"))
		{
			...
			alert("Please select Designation to delete");
	}
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT ID,Designation,Status, DATE_FORMAT(DateAdded, '%D %b %Y<br>%r') AS Added, 
				DATE_FORMAT(DateModified, '%D %b %Y<br>%r') AS Updated
				FROM designations WHERE ID <>0 ";
				if($Keywords != "")
					$query .= " AND (Designation LIKE '%" . dbinput($Keywords) . "%')";
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> Designations <small></small> </h1>
      <ol class="breadcrumb">
    ...
    <section class="content">
      <div class="row">
          ...
          <div class="box">
            <div class="box-header">
			  ...
			  <div class="buttons">
                ...
                <button class="btn btn-success margin" type="button" onClick="location.href='AddNewDesignation.php'">Add New</button>
		...
		?>
                    <tr class="noPrint">
                      <td colspan="11" align="center" class="error"><b>No Designation listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                    <tr>
                      ...
                      <td><a href="javascript:getPages(<?php echo $row["ID"]; ?>, 20)" state="1" class="main-page<?php echo $row["ID"]; ?>"><?php echo dboutput($row["Designation"]); ?></a></td>
					  <td><?php if(dboutput($row["Status"])=='1'){echo '<i class="fa fa-fw fa-check-circle"></i>';}else{echo '<i class="fa fa-fw fa-times-circle"></i>';} ?></td>
                      <td><?php echo total_employees_in_designation($row["Designation"]).' Employees Have This Designation'; ?></td>
					  <td><?php echo $row["Added"]; ?></td>
                      <td><?php echo $row["Updated"]; ?></td>
					  <td align="center" class="noPrint"><?php if($_SESSION['RoleID'] == 'Administrator'){ ?><button class="btn btn-info margin" type="button" onClick="location.href='EditDesignation.php?ID=<?php echo $row["ID"]; ?>'">Edit</button><?php } ?></td>
                    </tr>
                    <?php				
				}
			} 
