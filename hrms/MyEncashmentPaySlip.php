...
$query="SELECT p.ID AS EncashmentID,p.MonthEncashment,p.Heading,DATE_FORMAT(p.EncashmentDate, '%D %b %Y') AS EncashmentDate,e.FirstName,e.LastName,e.ID AS Employee,e.EmpID,e.Designation,e.Department,DATE_FORMAT(e.JoiningDate, '%D %b %Y') AS JoiningDate,c.Name AS Company,pd.TotalDays,pd.Basic,pd.Gross,pd.OtherAllowances,pd.BankorCash,pd.NetPay FROM encashmentdetails pd LEFT JOIN encashment p ON pd.EncashmentID = p.ID LEFT JOIN employees e ON pd.EmpID = e.ID LEFT JOIN companies c ON e.CompanyID = c.ID WHERE pd.ID='" . (int)$ID . "'";
	...
	else
	{
		...
		$EncashmentID=$row["EncashmentID"];
		$MonthEncashment=$row["MonthEncashment"];
		$EncashmentDate=$row["EncashmentDate"];
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <aside class="right-side">
	...
	<section class="content-header">
      <h1> Encashment Slip<small></small> </h1>
      <ol class="breadcrumb">
        <li><a href="MyEncashmentPaySlips.php"><i class="fa fa-dashboard"></i> My Encashment Pay Slips</a></li>
        <li class="active">My Encashment Slip</li>
      </ol>
    </section>
    <!-- Main content -->
		...
		<div class="col-lg-12 col-xs-12">
          ...
          <input type="hidden" id="ID" name="ID" value="<?php echo $ID; ?>" />
			...
			<div class="col-lg-12 col-xs-12">
				...
				<h6>Leave Encashment Generated <?php echo $EncashmentDate; ?> for (<?php echo $TotalDays; ?> Days)</h6>
			...
			<div class="col-lg-12 col-xs-12">
                  <table id="dataTable" class="table table-bordered invoice" style="background-color:white;">
                   ...
                   <tr >
                     <th >Annual Bonus Fixed Allowance</th>
                      <th ><span class="flo"><?php echo $OtherAllowances; ?></span></th>
					  
                   </tr>
				   <tr >
                     <th >Gross Salary with Annual Allowance</th>
                      <th ><span class="flo"><?php echo ($Gross+$OtherAllowances); ?></span></th>
					  
                   </tr>
				   <tr >
                     <th >Gross Salary without Annual Allowance</th>
