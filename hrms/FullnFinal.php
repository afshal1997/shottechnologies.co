...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			{
			mysql_query("DELETE FROM fullnfinal WHERE ID IN (" . $BID . ")") or die (mysql_error());
			...
			<b>Delete All selected Full n Final.</b>
			</div>';
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Please select Full n Final to delete.</b>
			</div>';
		}
	}
?>
...
<title>Full n Final</title>
<meta content='width=device-width, initial-scale=1, maximum-scale=1, Advance-scalable=no' name='viewport'>
<!-- bootstrap 3.0.2 -->
<!-- jQuery 2.0.2 -->
...
<script language="javascript">
	...
	function doDelete()
	{
		if($(".chkIds").is(":checked"))
		{
			...
			alert("Please select Full n Final to delete");
	}
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT d.ID,d.SpecialRemarks,d.GrandNetPay,e.EmpID,e.FirstName,e.LastName,e.Department,e.Designation,a.FirstName AS fn,a.LastName AS ln,a.Department AS dep,a.Designation AS des,DATE_FORMAT(d.DateAdded, '%D %b %Y') AS Added,DATE_FORMAT(d.DateModified, '%D %b %Y') AS Updated
				FROM fullnfinal d LEFT JOIN employees e ON d.EmpID = e.ID LEFT JOIN employees a ON d.PerformedBy = a.ID WHERE d.ID <>0 AND d.EmpID <>0 ".($Employee != 0 ? ' AND d.EmpID = '.$Employee.'' : '')." ".($CompanyID != '' ? ' AND FIND_IN_SET(e.CompanyID,\''.$CompanyID.'\')' : '')." ".($FromDate != "" && $TillDate != "" ? ' AND (d.DateAdded BETWEEN \''.$FromDate.'\' AND \''.$TillDate.'\')' : '')." ORDER BY d.DateAdded DESC";
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> Full n Final <small></small> </h1>
      <ol class="breadcrumb">
        <li><a href="Dashboard.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Full n Final</li>
      </ol>
    </section>
    <!-- Main content -->
    <section class="content">
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Full n Final</h3>
			  
			  <div class="buttons">
                
               
                <!--<button class="btn btn-primary margin no-print" id="btnExport"><i class="fa fa-download"></i> Download Excel</button>-->
				<button class="btn btn-success margin" type="button" onClick="location.href='AddNewFullnFinal.php'">Add Full n Final</button>
					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" class="margin">
						...
						<div class="col-lg-5 col-xs-12 no-print">
						  ...
						  Employee:
							...
							<?php
							 $query = "SELECT e.ID,e.EmpID,e.FirstName,e.LastName,e.Department,e.Designation FROM employees e LEFT JOIN fullnfinal f ON e.ID = f.EmpID where f.ID <> 0 ORDER BY e.ID ASC";
		...
		?>
                    <tr class="noPrint">
                      <td colspan="11" align="center" class="error"><b>No Full n Final listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                    <tr>
					  ...
					  <td><?php echo CURRENCY_SYMBOL.' '.dboutput($row["GrandNetPay"]); ?></td>
					  ...
					  <td align="center" class="noPrint"><button class="btn btn-info margin" type="button" onClick="location.href='FullnFinalSlip.php?ID=<?php echo $row["ID"]; ?>'">Slip</button></td>
					  <td align="center" class="noPrint"><button class="btn btn-info margin" type="button" onClick="location.href='EditFullnFinal.php?ID=<?php echo $row["ID"]; ?>'">Details</button></td>
                    </tr>
                    <?php				
				}
			} 
