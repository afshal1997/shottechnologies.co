...
if($action == "GrantLeaves")
	{
		...
		else
		{
			...
			<b>Please select Attendance Request for Leave Adjust.</b>
			</div>';
		}
	}
...
<head>
<meta charset="UTF-8">
<title>My Attendance Ledger</title>
...
<script language="javascript">
	...
	function GrantLeaves()
	{
		...
		{
			if(confirm("Are you sure to send Leaves Adjust Resuest."))
			{
			...
			alert("Please select Attendance to Leaves Adjust Resuest");
	}
	...
	function TimeAdjust()
	{
		if($(".chkIds").is(":checked"))
		{
			if(confirm("Are you sure to send Adjust Time Resuest."))
			{
			...
			alert("Please select Attendance to Adjust Time Resuest.");
	}
...
<?php
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1>My Attendance Ledger <small></small> </h1>
      <ol class="breadcrumb">
        <li><a href="Dashboard.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">My Attendance Ledger</li>
      </ol>
    </section>
    <!-- Main content -->
					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                       ...
                       <div class="col-lg-3 col-xs-12 no-print">
						...
						<select name="Employee" id="myselect" class="js-example-disabled-results form-control">
							<?php
							 $query = "SELECT ID,EmpID,FirstName,LastName,Department,Designation FROM employees where ID = ".$_SESSION['UserID']." ".($_SESSION['EmpTypeLedger'] != '' ? ' AND Status = "'.$_SESSION['EmpTypeLedger'].'"' : '')." ORDER BY ID ASC";
							$res = mysql_query($query);
							while($row = mysql_fetch_array($res))
							{
