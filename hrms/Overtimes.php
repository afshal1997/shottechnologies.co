...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			{
			mysql_query("DELETE FROM overtimes  WHERE ID IN (" . $BID . ")") or die (mysql_error());
			...
			<b>Delete All selected Overtimes.</b>
			</div>';
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Please select Overtime to delete.</b>
			</div>';
		}
	}
?>
...
<script language="javascript">
	...
	function doDelete()
	{
		if($(".chkIds").is(":checked"))
		{
			...
			alert("Please select Overtime to delete");
	}
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT d.ID,d.Approved,d.Hours,e.EmpID,e.FirstName,e.LastName,e.Department,e.Designation,a.FirstName AS fn,a.LastName AS ln,a.Department AS dep,a.Designation AS des,d.Title,DATE_FORMAT(d.Date, '%D %b %Y') AS Added
				FROM overtimes d LEFT JOIN employees e ON d.EmpID = e.ID LEFT JOIN employees a ON d.ApprovedBy = a.ID WHERE d.ID <>0 ";
				if($Keywords != "")
  ...
  <aside class="right-side">
    ...
    <section class="content">
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Overtimes</h3>
			  <div class="buttons">
                
                <button class="btn btn-success margin" type="button" onClick="location.href='AddNewOvertime.php'">Add New</button>
		...
		?>
                    <tr class="noPrint">
                      <td colspan="11" align="center" class="error"><b>No Overtime listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                    <tr>
					  ...
					  <td><?php echo dboutput($row["Hours"]); ?> Hours</td>
