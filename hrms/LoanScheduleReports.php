...
<?php
				...
				$query="SELECT e.EmpID,e.FirstName,e.LastName,c.Name AS Company,l.ID,l.Status,l.LoanType,l.Amount,l.RepaymentAmount,l.RemainingAmount,DATE_FORMAT(l.RepaymentDate, '%D %b %Y') AS RepaymentDate,DATE_FORMAT(l.DisbursementDate, '%D %b %Y') AS DisbursementDate FROM loans l LEFT JOIN employees e ON l.EmpID = e.ID LEFT JOIN companies c ON e.CompanyID = c.ID WHERE l.ID <> 0 AND (l.DisbursementDate BETWEEN '".date_format_Ymd($FromDate)."' AND '".date_format_Ymd($TillDate)."') ".($CompanyID != 0 ? ' AND e.CompanyID = '.$CompanyID.'' : '')." ".($Location != 0 ? ' AND e.Location = '.$Location.'' : '')." ".($LoanType != '' ? ' AND l.LoanType = "'.$LoanType.'"' : '')." ".($Employee != 0 ? ' AND e.ID = '.$Employee.'' : '')." ".($Department != "" ? ' AND e.Department = '.$Department.'' : '')." ".($Designation != "" ? ' AND e.Designation = '.$Designation.'' : '')." AND l.Status = 0 ORDER BY ".$SortBy." ASC";
		...
		?>
			...
			else
			{
				...
				<tr style="background-color:#c1c1c1;">
				  ...
				  <td><?php echo dboutput($row["RepaymentDate"]); ?></td>
				  <td><?php echo dboutput(number_format($row["Amount"],2)); ?></td>
				  <td>
				   <?php 
					$j=1;
					for($k = $row["RemainingAmount"]; $k > 0 ; $k = $k - $row["RepaymentAmount"])
					{
						$j++;
					}
					echo ($j-1);
				  ...
				  <td><?php echo dboutput(number_format($row["RemainingAmount"],2)); ?></td>
                </tr>
				<?php
					$query2 = "SELECT l.Amount,DATE_FORMAT(l.PaymentDate, '%D %b %Y') AS PaymentDate,l.PaymentType FROM loans_manualrecovery l where l.ID <> 0 AND l.LoanID='" . (int)$row['ID'] . "' Order By l.PaymentDate ASC";
						...
						while($row2 = mysql_fetch_array($res2))
						{
							?>
								...
								<tr>
								  ...
								  <td></td>
								  <td></td>
								  <td><?php echo $row2['PaymentDate']; ?></td>
								  <td><?php echo number_format($row2['Amount']); ?></td>
								  <td><?php echo '<mark style="background-color:#0f0">Manual Recovered</mark>'; ?></td>
								  <td></td>
								</tr>
							<?php
						}
					...
					?>
					<?php
					$query2 = "SELECT ID,Amount,DATE_FORMAT(RepaymentDate, '%D %b %Y') AS RepaymentDate,Status FROM loans_schedule where ID <> 0 AND LoanID='" . (int)$row['ID'] . "'";
					$res2 = mysql_query($query2) or die(mysql_error());
					$num2 = mysql_num_rows($res2);
					if($num2 > 0)
					{
						while($row2 = mysql_fetch_array($res2))
						{
							?>
								...
								<tr>
								  ...
								  <td></td>
								  <td></td>
								  <td><?php echo $row2['RepaymentDate']; ?></td>
								  <td><?php echo number_format($row2['Amount']); ?></td>
								  <td><?php if(dboutput($row2["Status"])=='1'){echo '<mark style="background-color:#0f0">Recovered</mark>';}else if(dboutput($row2["Status"])=='0'){echo '<mark style="background-color:#ff0">Running</mark>';}else{echo '<mark style="background-color:#f00">Stoped</mark>';} ?></td>
								  <td></td>
								</tr>
							<?php
						}
