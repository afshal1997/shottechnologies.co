...
// if(isset($_SESSION['Login']) && $_SESSION['Login']==true)
	...
	if(isset($_POST["action"]) && $_POST["action"] == "submit_form")
	{
		...
		{	
					...
					if($num == 0)
					{
						mysql_query("INSERT INTO user_login_history SET UserID = '".(int)$UserID."', LoginTime=CURRENT_TIME(), LoginDate=CURRENT_DATE()") or die(mysql_error());
						$id=mysql_insert_id();
						$res = mysql_query("SELECT DATE_FORMAT(LoginTime, '%T') AS ArrivalTime  FROM user_login_history WHERE ID = ".(int)$id."");
						$Rsc = mysql_fetch_assoc($res);
						$ArrivalTime = $Rsc['ArrivalTime'];
						$to_time=strtotime($ArrivalTime);
						$from_time=strtotime("09:30:00");
						// echo $from_time;
						$min = (int)round(($to_time - $from_time) / 60,2); 
						// echo round(($to_time - $from_time) / 60,2)." minute"; 
						// exit();
						if($min <= 0)
							{
							mysql_query("UPDATE user_login_history SET Status = 'OnTime' WHERE ID = ".(int)$id."") or die(mysql_error());
							
							$q="SELECT UserID FROM attendance WHERE  Year = '".date("Y")."' AND Month = '".date("F")."' AND UserID = '".(int)$UserID."'";
							$at = mysql_query($q);
							$numt = mysql_num_rows($at);
							if($numt == 0)
								mysql_query("INSERT INTO attendance SET OnTime = 1, UserID = '".(int)$UserID."', Year = '".date("Y")."' , Month = '".date("F")."' , NoOfDays = '1' ") or die(mysql_error());
							else
								mysql_query("UPDATE attendance SET NoOfDays = (NoOfDays + 1), OnTime = (OnTime + 1)  WHERE UserID = '".(int)$UserID."' AND Year = '".date("Y")."' AND Month = '".date("F")."' ") or die(mysql_error());
							}
						else
							{
							mysql_query("UPDATE user_login_history SET Status = 'Late'  WHERE ID = ".(int)$id."") or die(mysql_error());
							
							$q="SELECT UserID FROM attendance WHERE  Year = '".date("Y")."' AND Month = '".date("F")."' AND UserID = '".(int)$UserID."'";
							$at = mysql_query($q);
							$numt = mysql_num_rows($at);
							if($numt == 0)
								mysql_query("INSERT INTO attendance SET Late = 1, UserID = '".(int)$UserID."', Year = '".date("Y")."' , Month = '".date("F")."' , NoOfDays = '1' ") or die(mysql_error());
							else
								mysql_query("UPDATE attendance SET NoOfDays = (NoOfDays + 1), Late = (Late + 1)  WHERE UserID = '".(int)$UserID."' AND Year = '".date("Y")."' AND Month = '".date("F")."' ") or die(mysql_error());
					...
					//exit();
					if($num2 == 0)
						mysql_query("INSERT INTO user_logout_history SET UserID = '".(int)$UserID."', LogoutTime=CURRENT_TIME(), LogoutDate=CURRENT_DATE()") or die(mysql_error());
					else
...
$(document).ready(function(){
    $('input[type="radio"]').click(function(){
        if($(this).attr("value")=="arrival"){
            $("#departCheck").hide();
            $("#arrivalCheck").show();
        }
        if($(this).attr("value")=="depart"){
			$("#departCheck").show();
            $("#arrivalCheck").hide();
        }
    });
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
  <div class="biomatrics">
	...
	<img src="images/mirror.png" class="pull-right margin" style="position:relative;width: 35%;height: 29%;top: -36.5%;right: 20.3%;">
	<img src="images/atime.png" class="margin" style="position:relative;top: -37.5%;left:5%;">
	<img src="images/dtime.png" class="margin" style="position:relative;top: -81.5%;left:5%;">
	<img src="images/atCheck.png" id="arrivalCheck" class="margin" style="position:relative;top: -127.5%;left:6%;">
	<img src="images/dtCheck.png" id="departCheck" class="margin" style="position:relative;top: -128.5%;left:8%;display:none;">
	<input type="radio" name="checkit" id="arrival" value="arrival" checked style="position:relative;top: -131.5%;left:-10.5%;">
	<input type="radio" name="checkit" id="depart" value="depart" style="position:relative;top: -117.5%;left:-49%;">
 </div>
  </form>
  <?php 
