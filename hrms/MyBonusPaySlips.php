...
<head>
<meta charset="UTF-8">
<title>My Bonus Pay Slips</title>
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT pd.ID,pd.TotalDays,DATE_FORMAT(p.DateAdded, '%D %b %Y<br>%r') AS DateAdded,DATE_FORMAT(p.BonusDate, '%D %b %Y') AS BonusDate,p.MonthBonus FROM bonusdetails pd LEFT JOIN bonus p ON pd.BonusID = p.ID LEFT JOIN employees e ON pd.EmpID = e.ID WHERE pd.ID <>0 AND p.Steps = 3 AND e.ID = ".$_SESSION['UserID']." ";
				if($Keywords != "")
					$query .= " AND (p.MonthBonus LIKE '%" . dbinput($Keywords) . "%')";
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> My Bonus Pay Slips <small></small> </h1>
      <ol class="breadcrumb">
        <li><a href="Dashboard.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">My Bonus Pay Slips</li>
      </ol>
    </section>
    <!-- Main content -->
    <section class="content">
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">My Bonus Pay Slips</h3>
		...
		?>
                    <tr class="noPrint">
                      <td colspan="11" align="center" class="error"><b>No Bonus listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                    <tr>
					  ...
					  <td><?php echo $row["DateAdded"]; ?></td>
					   <td align="center" class="noPrint"><button class="btn btn-warning margin" type="button" onClick="location.href='MyBonusPaySlip.php?ID=<?php echo $row["ID"]; ?>'">Bonus Pay Slip</button></td>
                    </tr>
                    <?php				
				}
			} 
