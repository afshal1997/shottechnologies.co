...
// if($action == "delete")
	// {
		...
		// {
			...
			// {
			// mysql_query("DELETE FROM advances  WHERE ID IN (" . $BID . ")") or die (mysql_error());
			...
			// <b>Delete All selected Advances.</b>
			// </div>';
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT d.ID,d.Code,d.Status,d.Amount,DATE_FORMAT(d.RepaymentDate, '%D %b %Y') AS RepaymentDate,e.EmpID,e.FirstName,e.LastName,e.Department,e.Designation,a.FirstName AS fn,a.LastName AS ln,a.Department AS dep,a.Designation AS des,DATE_FORMAT(d.DateAdded, '%D %b %Y') AS Added,DATE_FORMAT(d.DateModified, '%D %b %Y') AS Updated
				FROM advances d LEFT JOIN employees e ON d.EmpID = e.ID LEFT JOIN employees a ON d.ApprovedBy = a.ID WHERE d.ID <>0 AND d.EmpID = ".$_SESSION['UserID']."";
				if($Keywords != "")
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> My Advances <small></small> </h1>
      <ol class="breadcrumb">
        <li><a href="Dashboard.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">My Advances</li>
      </ol>
    </section>
    <!-- Main content -->
