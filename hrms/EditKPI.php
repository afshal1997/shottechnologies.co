...
{	
	...
	if(isset($_POST["EmpID"]) && ctype_digit($_POST['EmpID']))
		...
		Status=1 AND ID <> '" . (int)$ID . "' AND EmpID = ".$EmpID."") or die(mysql_error());
	...
	if($msg=="")
	{
		...
		Name='".dbinput($Name)."' AND ID <> '".(int)$ID."' AND EmpID = ".$EmpID."") or die(mysql_error());
		if(mysql_num_rows($r) == 0)
		{
			$query="UPDATE kpi SET Status='".(int)$Status . "', DateModified=NOW(),
			Name = '" . dbinput($Name) . "',
			WS = '" . $WS . "'
			WHERE ID='".(int)$ID."'";
...
{
	$query="SELECT ID, Name,Status,WS,EmpID,DATE_FORMAT(DateAdded, '%D %b %Y %r') AS Added, DATE_FORMAT(DateModified,
	 '%D %b %Y %r') AS Updated FROM kpi WHERE  ID='" . (int)$ID . "'";
