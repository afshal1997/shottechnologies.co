...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			{
				mysql_query("DELETE FROM gazetted_holidays  WHERE ID IN (" . $BID . ")") or die (mysql_error());
			}
						
			$_SESSION["msg"]='<div class="alert alert-success alert-dismissable">
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Delete All selected Gazetted Holidays.</b>
			</div>';
			redirect("GazettedHolidays.php");
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Please select Gazetted Holiday to delete.</b>
			</div>';
		}
	}
?>
...
<script language="javascript">
	...
	function doDelete()
	{
		if($(".chkIds").is(":checked"))
		{
			...
			alert("Please select Gazetted Holiday to delete");
	}
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT d.ID,d.Approved,d.Title,a.FirstName AS fn,a.LastName AS ln,a.Department AS dep,a.Designation AS des,DATE_FORMAT(d.Date, '%D %b %Y') AS Added
				FROM gazetted_holidays d LEFT JOIN employees a ON d.ApprovedBy = a.ID WHERE d.ID <>0 ";
				if($Keywords != "")
					$query .= " AND (d.Title LIKE '%" . dbinput($Keywords) . "%')";
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> Gazetted Holidays <small></small> </h1>
      <ol class="breadcrumb">
        <li><a href="Dashboard.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Gazetted Holidays</li>
      </ol>
    </section>
    <!-- Main content -->
    <section class="content">
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Gazetted Holidays</h3>
			  <?php if($_SESSION['RoleID'] == 'HR' OR $_SESSION['RoleID'] == 'Administrator'){ ?>
			  <div class="buttons">
                
				<button class="btn btn-success margin" type="button" onClick="location.href='AddNewGazettedHoliday.php'">Add New</button>
		...
		?>
                    <tr class="noPrint">
                      <td colspan="11" align="center" class="error"><b>No Gazetted Holiday listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                    <tr>
					  ...
					  <?php if($_SESSION['RoleID'] == 'HR' OR $_SESSION['RoleID'] == 'Administrator'){ ?>
					  <td align="center" class="noPrint"><button class="btn btn-info margin" type="button" onClick="location.href='EditGazettedHoliday.php?ID=<?php echo $row["ID"]; ?>'">Edit</button></td>
					  <?php } ?>
                    </tr>
                    <?php				
				}
