...
if($action == "delete")
	{
		...
		{	
			...
			{
			$row = mysql_query("SELECT Resume FROM resumes  WHERE ID = ". $BID ."") or die (mysql_error());
			$dt = mysql_fetch_array($row);
			if(is_file(DIR_WEBSITE_CVS . $dt['Resume']))
				unlink(DIR_WEBSITE_CVS . $dt['Resume']);
			
			
			mysql_query("DELETE FROM resumes  WHERE ID IN (" . $BID . ")") or die (mysql_error());
			...
			<b>Delete All selected Resumes.</b>
			</div>';
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Please select Resume to delete.</b>
			</div>';
		}
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT r.ID,ro.PostName,r.Resume,DATE_FORMAT(r.DateAdded, '%D %b %Y<br>%r') AS Added	FROM resumes r LEFT JOIN jobposts ro ON r.PostID = ro.ID WHERE r.ID <>0 ";
				if($Keywords != "")
					$query .= " AND (ro.PostName LIKE '%" . dbinput($Keywords) . "%')";
  ...
  <aside class="right-side">
    ...
    <section class="content">
      <div class="row">
		...
		?>
                    <tr class="noPrint">
                      <td colspan="11" align="center" class="error"><b>No Resume listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                    <tr>
                      ...
                      <td><a href="javascript:getPages(<?php echo $row["ID"]; ?>, 20)" state="1" class="main-page<?php echo $row["ID"]; ?>"><?php echo $row["Added"]; ?></a></td>
					  <td>Res#: <?php echo $row["ID"]; ?></td>
