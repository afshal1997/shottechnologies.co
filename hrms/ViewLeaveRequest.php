	...
	if($msg=="")
	{
		...
		else
		{
			...
			mysql_query($query) or die (mysql_error());
				...
				{
					$query="SELECT AnualLeaves,CasualLeaves,SickLeaves FROM current_leaves_quota WHERE EmpID = ".(int)$LeaveSender."";
					$res = mysql_query($query) or die (mysql_error());
					$num = mysql_num_rows($res);
					if($num == 1)
					{
						$r = mysql_fetch_array($res);
						if($r['CasualLeaves'] > 0.9)
						{
							$query="UPDATE current_leaves_quota SET CasualLeaves = CasualLeaves - 1 WHERE EmpID='" . (int)$LeaveSender . "'";
							mysql_query($query) or die (mysql_error());
						...
						else if($r['SickLeaves'] > 0.9)
						{
							$query="UPDATE current_leaves_quota SET SickLeaves = SickLeaves - 1 WHERE EmpID='" . (int)$LeaveSender . "'";
							mysql_query($query) or die (mysql_error());
						...
						{
							$query="UPDATE current_leaves_quota SET AnualLeaves = AnualLeaves - 1 WHERE EmpID='" . (int)$LeaveSender . "'";
							mysql_query($query) or die (mysql_error());
