...
{	
	...
	if($msg=="")
	{
		$r = mysql_query("SELECT EmpID FROM leaves_quota WHERE 
		...
		if(mysql_num_rows($r) == 0)
		{
			...
			$query="UPDATE current_leaves_quota SET 
					...
					CasualLeaves = '" . (int)$C_CasualLeaves . "' WHERE 
					EmpID='".(int)$EmpID . "'";
			mysql_query($query) or die (mysql_error());
			
			$query="UPDATE leaves_quota SET 
			...
			$msg='<div class="alert alert-success alert-dismissable">
				<i class="fa fa-ban"></i>
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
				<b>Leaves Quota has been Updated.</b>
			</div>';
...
{
	$query="SELECT ID, Approved,ApprovedBy,EmpID,FrwdEmpID,AnualLeaves,SickLeaves,CasualLeaves,Date FROM leaves_quota WHERE  ID='" . (int)$ID . "'";
