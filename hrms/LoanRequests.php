...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			{
			mysql_query("DELETE FROM loan_requests  WHERE ID IN (" . $BID . ")") or die (mysql_error());
			...
			<b>Delete All selected Loan Requests.</b>
			</div>';
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Please select Loan Request to delete.</b>
			</div>';
		}
	}
?>
...
<script language="javascript">
	...
	function doDelete()
	{
		if($(".chkIds").is(":checked"))
		{
			...
			alert("Please select Loan Request to delete");
	}
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> Loan Requests <small></small> </h1>
      <ol class="breadcrumb">
      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
					...
					<form id="frmPages2" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" class="margin">
                        <div class="col-lg-12 col-xs-12">
							<div class="form-group no-print">
							  <label id="labelimp2" for="Role" >Loan Requests Filters: </label>
							</div>
                        </div><!-- ./col -->
			...
			?>
              <form id="frmPages"  method="post" action="<?php echo $_SERVER['PHP_SELF'] ;?>">
		...
		?>
                    <tr class="noPrint">
                      <td colspan="11" align="center" class="error"><b>No Loan Request listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                    <tr>
					  ...
					  <td><?php echo CURRENCY_SYMBOL.' '.dboutput($row["ActualAmount"]); ?></td>
					  ...
					  <td><?php if(dboutput($row["LoanGranted"])=='1'){echo '<mark style="background-color:#0f0">Yes</mark>';}else{echo '<mark style="background-color:#f00">No</mark>';} ?></td>
                      <td><?php echo dboutput($row['fn']).' '.dboutput($row['ln']).' ('.dboutput($row['dep']).' - '.dboutput($row['des']).')'; ?></td>
					  <td><?php echo $row["Added"]; ?></td>
					  <td><?php echo $row["Updated"]; ?></td>
					  <td align="center" class="no-Print no-print"><button class="btn btn-info margin no-print" type="button" onClick="location.href='LoanRequestSlip.php?ID=<?php echo $row["ID"]; ?>'">Slip</button></td>
					  <td align="center" class="no-Print no-print"><button class="btn btn-info margin" type="button" onClick="location.href='ViewLoanRequest.php?ID=<?php echo $row["ID"]; ?>'">Details</button></td>
                    </tr>
                    <?php				
				}
			} 
