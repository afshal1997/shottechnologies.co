...
include("CheckAdminLogin.php");
	...
	$query="SELECT s.DayNight FROM schedules s LEFT JOIN employees e ON s.ID = e.ScheduleID LEFT JOIN roster_login_history li ON li.UserID = e.ID  LEFT JOIN roster_logout_history lo ON li.UserID = lo.UserID AND li.ActualDate = lo.ActualDate WHERE li.ID = ".$_SESSION["TimeAdjust"][0]."";
	...
	if(isset($_SESSION["TimeAdjust"]) && is_array($_SESSION["TimeAdjust"]))
	{
		//echo 'working '.$_SESSION["TimeAdjust"][0]; exit();
		$numC = count($_SESSION["TimeAdjust"]);
		if($numC == 1)
		{
			if(user_arrivaltime($_SESSION["TimeAdjust"][0]) != '')
			{
				$ArrivalTime=user_arrivaltime($_SESSION["TimeAdjust"][0]);
			}
			else
			{
				$query="SELECT s.ArrivalTime,s.DayNight FROM schedules s LEFT JOIN employees e ON s.ID = e.ScheduleID LEFT JOIN roster_login_history li ON li.UserID = e.ID  LEFT JOIN roster_logout_history lo ON li.UserID = lo.UserID AND li.ActualDate = lo.ActualDate WHERE li.ID = ".$_SESSION["TimeAdjust"][0]."";
				$result = mysql_query($query) or die (mysql_error());
				$num = mysql_num_rows($result);
				if($num == 1)
				{
	...
	if(isset($_SESSION["TimeAdjust"]) && is_array($_SESSION["TimeAdjust"]))
	{
		...
		if($numC == 1)
		{
			if(user_departtime($_SESSION["TimeAdjust"][0]) != '')
			{
				$DepartTime=user_departtime($_SESSION["TimeAdjust"][0]);
			}
			else
			{
				$query="SELECT s.DepartTime,s.DayNight FROM schedules s LEFT JOIN employees e ON s.ID = e.ScheduleID LEFT JOIN roster_login_history li ON li.UserID = e.ID  LEFT JOIN roster_logout_history lo ON li.UserID = lo.UserID AND li.ActualDate = lo.ActualDate WHERE li.ID = ".$_SESSION["TimeAdjust"][0]."";
				$result = mysql_query($query) or die (mysql_error());
				$num = mysql_num_rows($result);
				if($num == 1)
				{
...
{			
	...
	if(isset($_POST["HalfDay"]))
		$HalfDay=trim($_POST["HalfDay"]);
	if(isset($_POST["Late"]))
		$Late=trim($_POST["Late"]);
	if(isset($_POST["EarlyDep"]))
		$EarlyDep=trim($_POST["EarlyDep"]);
	// echo $ArrivalTime.' '.$DepartTime;
	// exit();
	// if($Reason == "")
	// {
	...
	if($msg=="")
	{
		...
		if(isset($_SESSION["TimeAdjust"]) && is_array($_SESSION["TimeAdjust"]))
		{
			foreach($_SESSION["TimeAdjust"] as $TA)
			{
				
				$query5="SELECT e.ID AS EmpID,e.LeavesDays,lo.ID,li.LoginTime,li.DateAdded,lo.LogoutTime,li.Status
					 FROM employees e LEFT JOIN roster_login_history li ON li.UserID = e.ID  LEFT JOIN roster_logout_history lo ON li.UserID = lo.UserID AND li.ActualDate = lo.ActualDate WHERE li.ID = ".$TA."";
					 //echo $query5; exit();
				...
				// 	echo $nextdate; exit();
					...
					{
						$query2="UPDATE roster_login_history SET MLoginTime='".$row5['LoginTime']."',LoginTime='". time_format_gracetime($ArrivalTime) ."', HalfDay = 0,Late = 0,EarlyDep = 0, MStatus=CONCAT(IFNULL(MStatus,''), ' <br>".$Reason."') WHERE ID='" . (int)$TA . "'";
					...
					{
						$query2="UPDATE roster_login_history SET MLoginTime='".$row5['LoginTime']."',LoginTime='". time_format_gracetime($ArrivalTime) ."', Status = 'Present',HalfDay = 0,Late = 0,EarlyDep = 0, MStatus=CONCAT(IFNULL(MStatus,''), ' <br>".$Reason."') WHERE ID='" . (int)$TA . "'";
						mysql_query($query2) or die (mysql_error());
					...
					if($num == 1)
					{
						...
						else
						{
							...
							if (!in_array(''.date("N", $startdate).'', $HalfDays))
							{
								...
								{
									$query3="UPDATE roster_login_history SET MStatus = '".$Reason."' WHERE UserID='" . (int)$row['ID'] . "' AND ActualDate = '".$startdate."'";
									mysql_query($query3) or die (mysql_error());
								}
		...
		<b>Time Adjust of all selected Attendance.</b>
		</div>';
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Time Adjust</li>
      </ol>
    </section>
    <!-- Main content -->
