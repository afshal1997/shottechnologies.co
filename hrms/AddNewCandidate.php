...
{				
	...
	if(isset($_POST["ApplyedFor"]) && ctype_digit($_POST['ApplyedFor']))
		$ApplyedFor=trim($_POST["ApplyedFor"]);
	...
	if(isset($_POST["Phone"]))
		...
		else if($ApplyedFor == 0)
		{
			$msg='<div class="alert alert-danger alert-dismissable">
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Please select Applyed For.</b>
			</div>';
		}
		else if($Resume == 0)
		{
	...
	if($msg=="")
	{
		...
		$query="INSERT INTO candidates SET DateAdded=NOW(),
				EmailAddress = '" . dbinput($EmailAddress) . "',
				FirstName = '" . dbinput($FirstName) . "',
				LastName = '" . dbinput($LastName) . "',
				Status = '" . dbinput($Status) . "',
				ApplyedFor = '" . (int)$ApplyedFor . "',
				Resume = '" . (int)$Resume . "',
				IsShortlist = '" . dbinput($IsShortlist) . "',
				IsDisqualified = '" . dbinput($IsDisqualified) . "',
				Phone = '" . dbinput($Phone) . "'";
		...
		<i class="fa fa-ban"></i>
		<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
		<b>Candidate has been added.</b>
		</div>';
		
		redirect("AddNewCandidate.php");	
	}
...
<script src="//code.jquery.com/jquery-1.9.1.js"></script>
		<script>
			...
			$("#ApplyedFor").change(function () {
				...
				{
					$.ajax({			
							url: 'get_resume.php?PostID='+$("#ApplyedFor").val(),
          ...
          <div class="box">
            ...
            <div style="padding:15px;" class="box-primary">
              ...
              <div class="box-body">
				...
				<div class="form-group">
                  <label id="labelimp" for="ApplyedFor" >Applyed For: </label>
				  <select name="ApplyedFor" id="ApplyedFor" class="form-control">
					<option value="0" >Applyed For Post</option>
					<?php
					 $query = "SELECT ID,PostName FROM jobposts where Status = 1";
					$res = mysql_query($query);
					while($row = mysql_fetch_array($res))
					{
					echo '<option '.($ApplyedFor == $row['ID'] ? 'selected' : '').' value="'.$row['ID'].'">'.$row['PostName'].'</option>';
					} 
					?>
					</select>
                </div>
