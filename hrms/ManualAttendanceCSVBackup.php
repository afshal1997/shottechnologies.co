...
{	
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
		...
		{	
			...
			$EmpID = $EmpID['ID'];
			$validate="SELECT ID from user_login_history WHERE ID <> 0 AND UserID = '".$EmpID."' AND LoginDate = '".$data[1]."'";
			$result = mysql_query($validate) or die(mysql_error());
			$num = mysql_num_rows($result);
			if($num == 0)
			{
				$import="INSERT into user_login_history(UserID,LoginTime,LoginDate,Status,DateAdded) values('$EmpID','$data[2]','$data[1]','Present','$data[1]')";
				mysql_query($import) or die(mysql_error());
				
				$import="INSERT into user_logout_history(UserID,LogoutTime,LogoutDate,DateAdded) values('$EmpID','$data[3]','$data[1]','$data[1]')";
				mysql_query($import) or die(mysql_error());
			}
		}
    }
