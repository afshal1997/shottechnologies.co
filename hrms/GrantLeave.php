	...
	if($msg=="")
	{
		if($Type == "SpecialLeave")
		{
			if(isset($_SESSION["GrantLeave"]) && is_array($_SESSION["GrantLeave"]))
			{
				...
				{
				$query = "SELECT LoginDate,UserID,Status FROM roster_login_history WHERE ID IN (" . $GL . ")";
				...
				if($num > 0)
				{
					...
					if($row['Status'] == "Absent" || $row['Status'] == "Off Day")
					{
						$query2="UPDATE roster_login_history SET LastModified = ".$_SESSION['UserID'].",Updated=1,DateModified=NOW(),Status = 'Leave', MStatus=CONCAT(IFNULL(MStatus,''), ' <br>".dbinput($Reason)."') WHERE ID='" . (int)$GL . "'";
						mysql_query($query2) or die (mysql_error());
						
						$query2="INSERT INTO minus_leaves_quota SET EmpID = " . $row['UserID'] . ", LeaveDate='".$row['LoginDate']."', LeaveType = 'SpecialLeave', FromRoster = 1";
						mysql_query($query2) or die (mysql_error());
					}
				}
				...
				if($num > 0)
				{
					...
					if($row['Status'] == "Absent" || $row['Status'] == "Off Day")
					{
						...
						if($num3 > 0)
						{
							...
							if($row3['CasualLeaves'] > 0.9)
							{
								...
								LeaveType = 'CasualLeaves', FromRoster = 1";
								mysql_query($query2) or die (mysql_error());
							...
							else if($row3['SickLeaves'] > 0.9)
							{
								...
								$query2="INSERT INTO minus_leaves_quota SET EmpID = " . $row['UserID'] . ", LeaveDate='".$row['LoginDate']."',
								LeaveType = 'SickLeaves', FromRoster = 1";
								mysql_query($query2) or die (mysql_error());
							...
							else if($row3['AnualLeaves'] > 0.9)
							{
								...
								$query2="INSERT INTO minus_leaves_quota SET EmpID = " . $row['UserID'] . ", LeaveDate='".$row['LoginDate']."',
								LeaveType = 'AnualLeaves', FromRoster = 1";
								mysql_query($query2) or die (mysql_error());
					...
					else if($row['HalfDay'] == 1)
					{
						...
						if($num3 > 0)
						{
							...
							{
								$query2="UPDATE roster_login_history SET LastModified = ".$_SESSION['UserID'].",Updated=1,DateModified=NOW(),HalfDay = 0, MStatus=CONCAT(IFNULL(MStatus,''), ' <br>".dbinput($Reason)."') WHERE ID='" . (int)$GL . "'";
								mysql_query($query2) or die (mysql_error());
								
								$query2="INSERT INTO minus_leaves_quota SET EmpID = " . $row['UserID'] . ", LeaveDate='".$row['LoginDate']."',
								LeaveType = 'CasualLeaves', FromRoster = 1, Qty = 0.5";
								mysql_query($query2) or die (mysql_error());
							...
							else if($row3['SickLeaves'] > 0.4)
							{
								...
								$query2="INSERT INTO minus_leaves_quota SET EmpID = " . $row['UserID'] . ", LeaveDate='".$row['LoginDate']."',
								LeaveType = 'SickLeaves', FromRoster = 1, Qty = 0.5";
								mysql_query($query2) or die (mysql_error());
							...
							else if($row3['AnualLeaves'] > 0.4)
							{
								...
								$query2="INSERT INTO minus_leaves_quota SET EmpID = " . $row['UserID'] . ", LeaveDate='".$row['LoginDate']."',
								LeaveType = 'AnualLeaves', FromRoster = 1, Qty = 0.5";
								mysql_query($query2) or die (mysql_error());
