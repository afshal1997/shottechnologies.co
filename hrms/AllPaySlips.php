...
if(isset($_REQUEST["AllCompanies"]))
	$AllCompanies=trim($_REQUEST["AllCompanies"]);
if(isset($_REQUEST["AllLocations"]))
	$AllLocations=trim($_REQUEST["AllLocations"]);
if(isset($_REQUEST["AllDepartments"]))
	$AllDepartments=trim($_REQUEST["AllDepartments"]);
	
$query="SELECT ID FROM payroll WHERE ID='" . (int)$ID . "'";
					...
					<input type="hidden" id="ID" name="ID" value="<?php echo $ID; ?>" />
						<div class="col-lg-3 col-xs-12 no-print">
							<div class="form-group">
							  Company:
							  <select name="AllCompanies" id="AllCompanies" class="form-control">
								...
								{
								echo '<option '.($AllCompanies == $row['ID'] ? 'selected' : '').' value="'.$row['ID'].'">'.$row['Name'].'</option>';
								} 
								?>
								</select>
							</div>
						...
						<div class="col-lg-3 col-xs-12 no-print">
							<div class="form-group">
						  Location:
							<select name="AllLocations" id="AllLocations" class="form-control">
							...
							while($row = mysql_fetch_array($res))
							{
							echo '<option '.($AllLocations == $row['ID'] ? 'selected' : '').' value="'.$row['ID'].'">'.$row['Name'].' | '.$row['Abr'].'</option>';
							} 
							?>
							</select>
							</div>
						...
						<div class="col-lg-3 col-xs-12 no-print">
						  ...
						  Department:
							...
							{
							echo '<option '.($AllDepartments == $Departments ? 'selected' : '').' value="'.$Departments.'">'.$Departments.'</option>';
							} 
							?>
							</select>
						</div>
		...
		<div class="col-lg-12 col-xs-12">
          <br>
                  ...
                  <table id="dataTable" class="table table-bordered invoice" style="background-color:white;">
				   ...
				   <tr >
                     <td style="text-align:left">House Rent</td>
                      <td ><span class="flo"><?php echo round((40/100) * $Basic,2); ?></span></td>
			...
			<div class="col-lg-6 col-xs-6">
                  <table id="dataTable" class="table table-bordered invoice" style="background-color:white">
                    <tr >
                      <th width="80%">Other Allowances And Deductions</th>
					  <th align="right">Salary Structure</th>
                    </tr>
                   <tr >
