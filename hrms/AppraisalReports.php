...
<?php
// if($_SESSION['RoleID'] == 'Administrator' OR $_SESSION['RoleID'] == 'HR' OR user_authentication($_SESSION['UserID'],'LnAExeLoanRep') OR external_user_authentication($_SESSION['UserID'],'LnAExeLoanRep'))
...
if(isset($_REQUEST["Appraisals"]))
	$Appraisals=trim($_REQUEST["Appraisals"]);
...
<?php
				...
				$query="SELECT e.EmpID,e.FirstName,e.LastName,c.Name AS Company,l.ID,l.SupMarks FROM appraisals_result l LEFT JOIN employees e ON l.EmpID = e.ID LEFT JOIN companies c ON e.CompanyID = c.ID WHERE l.ID <> 0 ".($CompanyID != 0 ? ' AND e.CompanyID = '.$CompanyID.'' : '')." ".($Location != 0 ? ' AND e.Location = '.$Location.'' : '')." ".($Appraisals != 0 ? ' AND l.AppraisalID = "'.$Appraisals.'"' : '')." ".($Employee != 0 ? ' AND e.ID = '.$Employee.'' : '')." ".($Department != "" ? ' AND e.Department = '.$Department.'' : '')." ".($Designation != "" ? ' AND e.Designation = '.$Designation.'' : '')." AND (l.DateAdded BETWEEN '".date_format_Ymd($FromDate)." 00:00:00' AND '".date_format_Ymd($TillDate)." 23:59:59') ORDER BY ".$SortBy." ASC";
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1>Appraisal Reports <small></small> </h1>
      <ol class="breadcrumb">
        <li><a href="Dashboard.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Appraisal Reports</li>
      </ol>
    </section>
    <!-- Main content -->
    <section class="content">
      <div class="row">
          ...
          <br>
					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                        <div class="col-lg-12 col-xs-12">
                           <div class="form-group">
							  <label id="labelimp2" for="Role" >Appraisal Filters: </label>
							</div>
                        </div><!-- ./col -->
						...
						<div class="col-lg-3 col-xs-12 no-print">
							<div class="form-group">
						  Appraisals:
							<select name="Appraisals" id="Appraisals" class="form-control">
							<option value="" >All Appraisals</option>
							<?php
							$query = "SELECT ID,Title FROM appraisals where ID <> 0 ORDER BY Title ASC";
							$res = mysql_query($query);
							while($row = mysql_fetch_array($res))
							{
							echo '<option '.($Appraisals == $row['ID'] ? 'selected' : '').' value="'.$row['ID'].'">'.$row['Title'].'</option>';
							} 
							?>
							</select>
						</div>
			...
			<div class="box-footer no-print" style="text-align:right;">
               <!-- <button disabled type="submit" class="btn btn-default margin">Present</button>
				...
				<?php if($_SESSION['RoleID'] == 'Administrator' OR $_SESSION['RoleID'] == 'HR' OR user_authentication($_SESSION['UserID'],'LnAPrntLoanRep') OR external_user_authentication($_SESSION['UserID'],'LnAPrntLoanRep')){ ?>
              ...
              <table class="blue table table-bordered">
                  <thead class="">
                    <tr style="color:white;">
					  ...
					  <th>Score</th>
					  <?php
					  $query2="SELECT Name FROM rate_analysis WHERE Status = 1 ORDER BY Name ASC";
		...
		?>
                    <tr class="noPrint">
                      <td colspan="20" align="center" class="error"><b>No Appraisal listed.</b></td>
                    </tr>
                    <?php 
			}
			else
			{
				...
				<tr>
				  ...
				  <td><?php echo ($row["SupMarks"] > 0 ? $row["SupMarks"] : 'Not Rate Yet'); ?></td>
				  <?php
				  $query2="SELECT Name,One,Two,Three,Four,Five FROM rate_analysis WHERE Status = 1 ORDER BY Name ASC";
				  $result2 = mysql_query ($query2) or die(mysql_error()); 
				  $num2 = mysql_num_rows($result2);
					if($num2 > 0)
					{
						while($row2 = mysql_fetch_array($result2,MYSQL_ASSOC))
						{
							if($row["SupMarks"] > 4)
							{
								echo '<td>'.dboutput($row2["Five"]).'</td>';
							}
							else if($row["SupMarks"] > 3)
							{
								echo '<td>'.dboutput($row2["Four"]).'</td>';
							}
							else if($row["SupMarks"] > 2)
							{
								echo '<td>'.dboutput($row2["Three"]).'</td>';
							}
							else if($row["SupMarks"] > 1)
							{
								echo '<td>'.dboutput($row2["Two"]).'</td>';
							}
							else if($row["SupMarks"] > 0)
							{
								echo '<td>'.dboutput($row2["One"]).'</td>';
							}
							else
							{
								echo '<td style="background-color:red;color:yellow;">Undefine</td>';
							}
						}
					}
				  ?>
