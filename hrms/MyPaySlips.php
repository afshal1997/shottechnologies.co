...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT pd.ID,p.NumOfDays,DATE_FORMAT(p.DateAdded, '%D %b %Y<br>%r') AS DateAdded,DATE_FORMAT(p.ToDate, '%D %b %Y') AS ToDate,DATE_FORMAT(p.FromDate, '%D %b %Y') AS FromDate,p.MonthPayroll	FROM payrolldetails pd LEFT JOIN payroll p ON pd.PayID = p.ID LEFT JOIN employees e ON pd.EmpID = e.ID WHERE pd.ID <>0 AND p.Steps = 3 AND e.ID = ".$_SESSION['UserID']." ";
				if($Keywords != "")
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> My Pay Slips <small></small> </h1>
      <ol class="breadcrumb">
        <li><a href="Dashboard.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">My Pay Slips</li>
      </ol>
    </section>
    <!-- Main content -->
		...
		?>
                    <tr>
					  ...
					  <td><?php echo $row["DateAdded"]; ?></td>
					   <td align="center" class="noPrint"><button class="btn btn-warning margin" type="button" onClick="location.href='MyPaySlip.php?ID=<?php echo $row["ID"]; ?>'">Pay Slip</button></td>
                    </tr>
                    <?php				
				}
			} 
