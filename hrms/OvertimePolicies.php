...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			{
			mysql_query("DELETE FROM overtime_policies  WHERE ID IN (" . $BID . ")") or die (mysql_error());
			...
			<b>Delete All selected Overtime Policies.</b>
			</div>';
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Please select Overtime Policy to delete.</b>
			</div>';
		}
	}
?>
...
<head>
<meta charset="UTF-8">
<title>Overtime Policies</title>
<meta content='width=device-width, initial-scale=1, maximum-scale=1, Overtime-scalable=no' name='viewport'>
<!-- bootstrap 3.0.2 -->
<!-- jQuery 2.0.2 -->
...
<script language="javascript">
	...
	function doDelete()
	{
		if($(".chkIds").is(":checked"))
		{
			...
			alert("Please select Overtime Policy to delete");
	}
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT ID,Status,Name,ApplyAfter,OTHourType,OTHourBase,OTHourValue,OTHolidayType,OTHolidayBase,OTHolidayValue,DATE_FORMAT(DateAdded, '%D %b %Y<br>%r') AS Added,DATE_FORMAT(DateModified, '%D %b %Y<br>%r') AS Updated
				FROM overtime_policies WHERE ID <>0 ";
				if($Keywords != "")
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> Overtime Policies <small></small> </h1>
      <ol class="breadcrumb">
        <li><a href="Dashboard.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Overtime Policies</li>
      </ol>
    </section>
    <!-- Main content -->
    <section class="content">
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Overtime Policies</h3>
			  <div class="buttons">
                <?php if($_SESSION['RoleID'] == 'HR' OR $_SESSION['RoleID'] == 'Administrator'){ ?>
                <button class="btn btn-success margin" type="button" onClick="location.href='AddNewOvertimePolicy.php'">Add New</button>
		...
		?>
                    <tr class="noPrint">
                      <td colspan="15" align="center" class="error"><b>No Overtime Policy listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                    <tr>
					  ...
					  <td><?php echo dboutput($row['ApplyAfter']); ?> Minutes</td>
					  <td><?php echo dboutput($row["OTHourType"]); ?></td>
					  <td><?php echo dboutput($row["OTHourBase"]); ?></td>
					  <td><?php echo dboutput($row["OTHourValue"]); ?></td>
					  <td><?php echo dboutput($row["OTHolidayType"]); ?></td>
					  <td><?php echo dboutput($row["OTHolidayBase"]); ?></td>
					  <td><?php echo dboutput($row["OTHolidayValue"]); ?></td>
                      <td><?php if(dboutput($row["Status"])==1){echo '<i class="fa fa-fw fa-check-circle"></i>';}else{echo '<i class="fa fa-fw fa-times-circle"></i>';} ?></td>
                      <td><?php echo $row["Added"]; ?></td>
                      <td><?php echo $row["Updated"]; ?></td>
					  <td align="center" class="noPrint"><button class="btn btn-info margin" type="button" onClick="location.href='EditOvertimePolicy.php?ID=<?php echo $row["ID"]; ?>'">Edit</button></td>
                    </tr>
                    <?php				
				}
			} 
