...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			{
				$query = "SELECT ID,EmpID,LeaveType,Qty FROM minus_leaves_quota where ID IN (" . $BID . ")";
				...
				if($num == 1)
				{
					...
					if($row['LeaveType'] != "SpecialLeave")
					{
						mysql_query("UPDATE current_leaves_quota SET ".$row['LeaveType']." = (".$row['LeaveType']." + ".$row['Qty'].") WHERE EmpID = ".$row['EmpID']."") or die (mysql_error());
					}
					
					mysql_query("DELETE FROM minus_leaves_quota  WHERE ID IN (" . $BID . ")") or die (mysql_error());
				}
			...
			<b>Delete All selected Leaves.</b>
			</div>';
			redirect("Leaves.php");
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT d.ID,d.LeaveType,DATE_FORMAT(d.LeaveDate, '%D %b %Y') AS LeaveDate,e.EmpID,e.FirstName,e.LastName,e.Department,e.Designation
				FROM minus_leaves_quota d LEFT JOIN employees e ON d.EmpID = e.ID WHERE d.ID <>0 ".($Employee != 0 ? ' AND d.EmpID = '.$Employee.'' : '')." ".($FromDate != "" && $TillDate != "" ? ' AND (d.LeaveDate BETWEEN \''.$FromDate.'\' AND \''.$TillDate.'\')' : '')." ORDER BY d.LeaveDate DESC";
				// if($Keywords != "")
  ...
  <aside class="right-side">
    ...
    <section class="content">
      <div class="row">
          ...
          <div class="box">
            <div class="box-header">
			  ...
			  <div class="buttons">
                ...
                <button class="btn btn-success margin" type="button" onClick="location.href='AddManualLeave.php'">Add Manual Leave</button>
		...
		?>
                    <tr class="noPrint">
                      <td colspan="11" align="center" class="error"><b>No Leave listed.</b></td>
                    </tr>
                    <?php 
			}
			else
