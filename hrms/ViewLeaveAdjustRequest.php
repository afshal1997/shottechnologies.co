...
{			
	...
	if($msg=="")
	{
		...
		if($Approval == 2)
		{
			...
			$query="UPDATE leaveadjust_requests SET 
				Approved = '".(int)$Approval."',
				DisapproveReason = '" . dbinput($DisapproveReason) . "' WHERE ID = ".$ID."";
			mysql_query($query) or die (mysql_error());
			$ID = mysql_insert_id();
			$_SESSION["msg"]='<div class="alert alert-success alert-dismissable">
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Leave Adjust request has been updated.</b>
			</div>';		
		...
		else
		{
			...
			{
				$query = "SELECT Status FROM roster_login_history WHERE ID=" . $RosterID . "";
				...
				if($num > 0)
				{
					...
					{
						$query2="UPDATE roster_login_history SET Status = 'Leave', MStatus='".$Reason."' WHERE ID='" . (int)$RosterID . "'";
						mysql_query($query2) or die (mysql_error());
					}
				}
				$_SESSION["msg"]='<div class="alert alert-success alert-dismissable">
			...
			{
				$query = "SELECT UserID,HalfDay,Status,LoginDate FROM roster_login_history WHERE ID=" . $RosterID . "";
				$result = mysql_query($query) or die (mysql_error());
				$num = mysql_num_rows($result);
				...
				if($num > 0)
				{
					...
					else if($row['HalfDay'] == 1)
					{
						...
						if($num3 > 0)
						{
							...
							{
								$query2="UPDATE roster_login_history SET  HalfDay = 0, MStatus='".$Reason."' WHERE ID='" . (int)$RosterID . "'";
								mysql_query($query2) or die (mysql_error());
...
{
	$query="SELECT l.ID,l.RosterID,l.Reason,l.Approved,l.DisapproveReason,l.Adjust,l.Type,DATE_FORMAT(l.Date, '%D %b %Y') AS Date,e.EmpID,e.FirstName,e.LastName,e.Department,e.Designation,DATE_FORMAT(l.DateAdded, '%D %b %Y') AS Added
	FROM leaveadjust_requests l LEFT JOIN employees e ON l.EmpID = e.ID WHERE  l.ID='" . (int)$ID . "'";
	...
	else
	{
		...
		$Approval=dboutput($rows['Approved']);
		$RosterID=dboutput($rows['RosterID']);
		$EmpID=dboutput($rows['EmpID']);
		$FirstName=dboutput($rows['FirstName']);
		$LastName=dboutput($rows['LastName']);
		$Type=dboutput($rows['Type']);
		$NumOfDays=dboutput($rows['Adjust']);
		$Date=dboutput($rows['Date']);
...
<head>
<meta charset="UTF-8">
<title>View Leave Adjust Request</title>
...
?>
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> View Leave Adjust Request</h1>
      <ol class="breadcrumb">
        <li><a href="LeaveAdjustRequests.php"><i class="fa fa-dashboard"></i>Leave Adjust Requests</a></li>
        <li class="active">View Leave Adjust Request</li>
      </ol>
    </section>
    <!-- Main content -->
    <form role="form" action="<?php echo $_SERVER['PHP_SELF']; ?>?ID=<?php echo $ID; ?>" method="post" enctype="multipart/form-data" name="frmPage">
      <section class="content">
       <div class="box-footer" style="text-align:right;">
                ...
                <button class="btn btn-primary margin" type="button" onClick="location.href='LeaveAdjustRequests.php'">Cancel</button>
            </div>
              <?php
		  		echo $msg;
          ...
          <div class="box">
            ...
            <div style="padding:15px;" class="box-primary">
              ...
              <div class="box-body">
				...
				echo '<input type="hidden" class="form-control" name="RosterID" value="'.$RosterID.'" />';
				?>
                </div>
				
				<div class="form-group">
                  <label id="labelimp" >Adjustment Type: </label>
                  <?php 
				...
				<div class="form-group">
                  <label id="labelimp" >Adjustment Date: </label>
                  <?php 
				echo '<input type="text" class="form-control" disabled value="'.$Date.'" />';
				?>
                </div>
