...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			$queryCheck="SELECT ID FROM payroll WHERE ID IN (" . $BID . ") AND Steps = 0";
			$resultCheck = mysql_query ($queryCheck) or die(mysql_error()); 
			$numCheck = mysql_num_rows($resultCheck);
			if($numCheck == 1)
			{
			
			$query="SELECT LeaveType,EmpID FROM minus_leaves_quota_payroll WHERE PayID IN (" . $BID . ")";
			...
			$query="SELECT AdvID FROM payrolladvancedetails WHERE PayID IN (" . $BID . ")";
			$result = mysql_query ($query) or die(mysql_error()); 
			$num = mysql_num_rows($result);
			if($num > 0)
			{
			...
			$query="SELECT LoanID,LoanScheduleID,Amount FROM payrollloandetails WHERE PayID IN (" . $BID . ")";
			$result = mysql_query ($query) or die(mysql_error()); 
			$num = mysql_num_rows($result);
			if($num > 0)
			{
			...
			mysql_query("DELETE FROM minus_leaves_quota_payroll WHERE PayID IN (" . $BID . ")") or die (mysql_error());
			mysql_query("DELETE FROM payrolladvancedetails WHERE PayID IN (" . $BID . ")") or die (mysql_error());
			mysql_query("DELETE FROM payrollloandetails WHERE PayID IN (" . $BID . ")") or die (mysql_error());
			mysql_query("DELETE FROM payrollallowancedetails WHERE PayID IN (" . $BID . ")") or die (mysql_error());
			mysql_query("DELETE FROM payrolldeductiondetails WHERE PayID IN (" . $BID . ")") or die (mysql_error());
			mysql_query("DELETE FROM payrollcontributiondetails WHERE PayID IN (" . $BID . ")") or die (mysql_error());
			mysql_query("DELETE FROM payroll WHERE ID IN (" . $BID . ")") or die (mysql_error());
			mysql_query("DELETE FROM payrolldetails  WHERE PayID IN (" . $BID . ")") or die (mysql_error());
			mysql_query("DELETE FROM payrolltaxes  WHERE PayID IN (" . $BID . ")") or die (mysql_error());
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
			<b>Please select Payroll to delete.</b>
			</div>';
		}
	}
?>
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				FROM payroll p LEFT JOIN employees e ON p.PerformedBy = e.ID WHERE p.ID <>0 ".($_SESSION['PayrollMonthAuditReport'] != "" ? ' AND p.MonthPayroll = \''.$_SESSION['PayrollMonthAuditReport'].'\'' : '')." ".($_SESSION['PayrollCompanyIDAuditReport'] != "" ? ' AND FIND_IN_SET(p.CompanyID,\''.$_SESSION['PayrollCompanyIDAuditReport'].'\')' : '')." ORDER BY p.ID DESC";
  ...
  <aside class="right-side">
    ...
    <section class="content">
      <div class="row">
		...
		?>
                    <tr class="noPrint">
                      <td colspan="13" align="center" class="error"><b>No Payroll listed.</b></td>
                    </tr>
                    <?php 
			}
			else
		...
		?>
                    <tr>
					  ...
					  <td><?php echo $row["DateAdded"]; ?></td>
					   ...
					   <td align="center" class="noPrint"><button <?php echo ($row["Steps"] != 3 ? 'disabled' : ''); ?> class="btn btn-warning margin" type="button" onClick="location.href='PaySlips.php?ID=<?php echo $row["ID"]; ?>'">Pay Slips</button></td>
