...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT tr.ID,t.ID AS TestID, tr.Title, t.TestName,tr.StartTime,tr.EndTime, tr.StartDate,tr.EndDate FROM trainings tr LEFT JOIN tests t ON tr.Test = t.ID WHERE tr.ID <> 0 AND FIND_IN_SET (".$Employee.", tr.Employees)";
				if($Keywords != "")
      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Trainings In My Supervision</h3>
					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                        <div class="col-lg-3 col-xs-12">
                           <div class="form-group">
							  <label id="labelimp2" for="Role" >Trainings Filters: </label>
							</div>
                        </div><!-- ./col -->
							...
							<div class="form-group">
							  <select name="Employee" id="Employee" class="form-control">
								<option value="0" >Select Employee</option>
									<?php
									 $query = "SELECT ID,EmpID,FirstName,LastName,Department,Designation FROM employees where ID <> 0 AND Supervisor = '".empCodeByID($_SESSION['UserID'])."' ORDER BY Department,Designation ASC";
		...
		?>
                    <tr>
					  ...
					  <td>
					  <?php 
							$sql2 = "SELECT ID, StudentID, TestID, Grade, Attempts FROM stud_answers WHERE StudentID = '".$Employee."' AND TestID = '".$row['TestID']."'";
							$da2 = mysql_query($sql2);
							$ro2 = mysql_fetch_assoc($da2);
							if($ro2['StudentID'] == $Employee && $ro2['TestID'] == $row['TestID']&& ($ro2['Grade'] == 'Passed' || $ro2['Attempts'] == NUM_OF_TEST_ATTEMPTS))
							{
								...
								echo ((date('Y-m-d', strtotime(date('Y-m-d'))) >= date('Y-m-d', strtotime($strDate))) && (date('Y-m-d', strtotime(date('Y-m-d'))) <= date('Y-m-d', strtotime($enDate))) ? '<a class="btn bg-navy" href="#">Not Given Yet</a>' : '<span class="btn btn-danger">'.((date('Y-m-d', strtotime(date('Y-m-d'))) > date('Y-m-d', strtotime($enDate))) ? 'Test Missed' : 'Pending').'</span>');
							}
							?>
					  </td>
                    </tr>
