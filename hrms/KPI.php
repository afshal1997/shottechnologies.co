...
if($action == "delete")
	{
		if(isset($_REQUEST["ids"]) && is_array($_REQUEST["ids"]))
		{
			...
			mysql_query("DELETE FROM kpi  WHERE ID IN (" . $ids . ")") or die (mysql_error());
						
			$_SESSION["msg"]='<div class="alert alert-success alert-dismissable">
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
			<b>Delete All selected KPI.</b>
			</div>';
			redirect("KPI.php");
		...
		else
		{
			...
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
			<b>Please select KPI to delete.</b>
			</div>';
		}
	}
?>
...
<script language="javascript">
	...
	function doDelete()
	{
		if($(".chkIds").is(":checked"))
		{
			...
			alert("Please select KPI to delete");
	}
...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				// $query="SELECT ID,Name,WS,Status, DATE_FORMAT(DateAdded, '%D %b %Y<br>%r') AS Added, 
				// DATE_FORMAT(DateModified, '%D %b %Y<br>%r') AS Updated
				// FROM KPI WHERE ID <>0 ";
				// if($Keywords != "")
					// $query .= " AND (Name LIKE '%" . dbinput($Keywords) . "%')";
				
				$query="SELECT e.ID,c.Name,e.Status,e.Photo,e.EmpID,e.FirstName,e.LastName,e.Department,e.Designation FROM employees e LEFT JOIN companies c ON e.CompanyID = c.ID WHERE e.ID <> 0 AND e.ID IN (Select EmpID from kpi)";	
				if($Keywords != "")
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> KPI's <small></small> </h1>
      <ol class="breadcrumb">
        <li><a href="Dashboard.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">KPI's</li>
      </ol>
    </section>
    <!-- Main content -->
    <section class="content">
      <div class="row">
          ...
          <div class="box">
            <div class="box-header">
			  ...
			  <div class="buttons">
                ...
                <button class="btn btn-success margin" type="button" onClick="location.href='AddNewKPI.php'">Add New</button>
                <?php } ?>
		...
		?>
                    <tr class="noPrint">
                      <td colspan="11" align="center" class="error"><b>No KPI listed.</b></td>
                    </tr>
                    <?php 
			}
			else
			{
				...
				{
		?>
                    <tr style="background-color:<?php echo kpi_total($row["ID"]); ?>">
                      <td><b>Name:</b><br><?php echo dboutput($row["FirstName"]) ." ". dboutput($row["LastName"]); ?></td>
					  <td><b>Photo:</b><br><img class="thumbnail" src="<?php echo (is_file(DIR_EMPLOYEEPHOTOES . $row['Photo']) ? DIR_EMPLOYEEPHOTOES.dboutput($row['Photo']) : 'images/avatar.png'); ?>"  style="width:80px;height: 80px;" /></td>
					  <td><b>Employee ID:</b><br><?php echo dboutput($row["EmpID"]); ?></td>
					  <td><b>Company:</b><br><?php echo dboutput($row["Name"]); ?></td>
					  <td><b>Department:</b><br><?php echo dboutput($row["Department"]); ?></td>
					  <td><b>Designation:</b><br><?php echo dboutput($row["Designation"]); ?></td>
					 <td><b>Status:</b><br><?php if(dboutput($row["Status"])=='Active'){echo '<span style="background-color:green;color:white">&nbsp;Active&nbsp;</span>';}else{echo '<span style="background-color:red;color:white">&nbsp;Deactive&nbsp;</span>';} ?></td>
                    </tr>
                    <?php
					$query2="SELECT ID,Name,WS,Status, DATE_FORMAT(DateAdded, '%D %b %Y<br>%r') AS Added, 
					DATE_FORMAT(DateModified, '%D %b %Y<br>%r') AS Updated
					FROM kpi WHERE ID <> 0 AND EmpID = " . dboutput($row["ID"]) . "";
					$result2 = mysql_query ($query2) or die("Could not select because: ".mysql_error());
					while($row2 = mysql_fetch_array($result2,MYSQL_ASSOC))
					{
					?>
					<tr>
                      <td align="center" class="noPrint"><input class="chkIds" type="checkbox" name="ids[]" value="<?php echo $row2["ID"]; ?>" />
                        <input type="hidden" name="ids1[]" value="<?php echo $row2["ID"]; ?>"></td>
                      <td><?php echo dboutput($row2["Name"]); ?></td>
					  <td><?php echo dboutput($row2["WS"]); ?></td>
					  <td><?php if(dboutput($row2["Status"])=='1'){echo '<i class="fa fa-fw fa-check-circle"></i>';}else{echo '<i class="fa fa-fw fa-times-circle"></i>';} ?></td>
					  <td><?php echo $row2["Added"]; ?></td>
                      <td><?php echo $row2["Updated"]; ?></td>
					  <td align="center" class="noPrint">
					  <?php if($_SESSION['RoleID'] == 'HR' OR $_SESSION['RoleID'] == 'Administrator'){ ?>
					  <button class="btn btn-info margin" type="button" onClick="location.href='EditKPI.php?ID=<?php echo $row2["ID"]; ?>'">Edit</button>
					  <?php } ?>
					  </td>
                    </tr>
					<?php
