...
{		
	...
	if($msg=="")
	{
		...
		for($i=0; $i<$num_of_days;$i++)
		{
			...
			$query="SELECT ID,LeavesDays,HalfDays,ScheduleID,LateArrivalPolicy,EarlyDeparturePolicy,ArrivalHalfDay,DepartHalfDay FROM employees WHERE Status='Active'";
			$result = mysql_query ($query) or die(mysql_error()); 
			$num = mysql_num_rows($result);
			while($row = mysql_fetch_array($result,MYSQL_ASSOC))
			{
				$query2="SELECT ID,LoginTime FROM user_login_history WHERE UserID='" . (int)$row['ID'] . "' AND DateAdded = '".date("Y-m-d", $startdate)."'";
				...
				if($num2 == 0)
				{
					...
					if ($num3 != 0)
					{
					$query3="INSERT INTO user_login_history SET LoginDate='".date("Y-m-d", $startdate)."' , DateAdded='".date("Y-m-d", $startdate)."',
						UserID = '" . (int)$row['ID'] . "',
						Status = 'Leave'";
					mysql_query($query3) or die (mysql_error());
					
					$query3="INSERT INTO user_logout_history SET DateAdded='".date("Y-m-d", $startdate)."',
						UserID = '" . (int)$row['ID'] . "'";
					mysql_query($query3) or die (mysql_error());
					}
				...
				else
				{
					...
					$query3="SELECT ArrivalTime,LateArrival,ArrivalHalfDay FROM schedules WHERE ID='" . (int)$row['ScheduleID'] . "'";
					$result3 = mysql_query ($query3) or die(mysql_error()); 
					$num3 = mysql_num_rows($result3);
					if($num3 == 1)
					{
						$row3 = mysql_fetch_array($result3);
						$ArrivalTime=$row3['ArrivalTime'];
						$LateArrival=$row3['LateArrival'];
						$ArrivalHalfDay=$row3['ArrivalHalfDay'];
					}
					
					
					if (!in_array(''.date("N", $startdate).'', $HalfDays))
					{
						if ($row3['ArrivalHalfDay'] == "Not Allowed")
						{
							if ($row2['LoginTime'] > $ArrivalHalfDay)
							{
							$query3="UPDATE user_login_history SET HalfDay = 1 WHERE UserID='" . (int)$row['ID'] . "' AND DateAdded = '".date("Y-m-d", $startdate)."'";
							mysql_query($query3) or die (mysql_error());
							}
						}
						else if ($row3['LateArrival'] == "Not Allowed")
						{
							if ($row2['LoginTime'] > $LateArrival)
							{
							$query3="UPDATE user_login_history SET Late = 1 WHERE UserID='" . (int)$row['ID'] . "' AND DateAdded = '".date("Y-m-d", $startdate)."'";
							mysql_query($query3) or die (mysql_error());
							}
						}
					}
				...
				// Arrival End
				$query2="SELECT ID,LogoutTime FROM user_logout_history WHERE UserID='" . (int)$row['ID'] . "' AND DateAdded = '".date("Y-m-d", $startdate)."'";
				$result2 = mysql_query ($query2) or die(mysql_error()); 
				$num2 = mysql_num_rows($result2);
				$row2 = mysql_fetch_array($result2);
				...
				else
				{
					...
					$query3="SELECT DepartTime,EarlyDepart,DepartHalfDay FROM schedules WHERE ID='" . (int)$row['ScheduleID'] . "'";
					$result3 = mysql_query ($query3) or die(mysql_error()); 
					$num3 = mysql_num_rows($result3);
					if($num3 == 1)
					{
						$row3 = mysql_fetch_array($result3);
						$DepartTime=$row3['DepartTime'];
						$EarlyDepart=$row3['EarlyDepart'];
						$DepartHalfDay=$row3['DepartHalfDay'];
					}
					
					
					if (!in_array(''.date("N", $startdate).'', $HalfDays))
					{
						if ($row3['DepartHalfDay'] == "Not Allowed")
						{
						...
						else if ($row3['EarlyDepart'] == "Not Allowed")
						{
							if ($row2['LogoutTime'] < $EarlyDeparturePolicy)
							{
							$query3="UPDATE user_login_history SET EarlyDep = 1 WHERE UserID='" . (int)$row['ID'] . "' AND DateAdded = '".date("Y-m-d", $startdate)."'";
							mysql_query($query3) or die (mysql_error());
							}
						}
					}
              ...
              <div class="box-body">
				...
				<div class="form-group">
				  ...
				  <select name="What" id="What" class="form-control">
					...
					<option <?php echo ($What == 'Full day On' ? 'selected' : ''); ?> value="Full day On">Full day On (Office Open)</option>
					</select>
				</div>
