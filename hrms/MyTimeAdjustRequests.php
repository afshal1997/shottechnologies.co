...
<div class="wrapper row-offcanvas row-offcanvas-left">
  ...
  <?php
		include_once("Sidebar.php");
				...
				$query="SELECT l.ID,l.RosterID,DATE_FORMAT(r.LoginDate, '%D %b %Y') AS AdjustDate,l.Approved,l.LoginTime,l.LogoutTime,e.EmpID,e.FirstName,e.LastName,e.Department,e.Designation,a.FirstName AS fn,a.LastName AS ln,a.Department AS dep,a.Designation AS des,DATE_FORMAT(l.DateAdded, '%D %b %Y') AS Added,DATE_FORMAT(l.DateModified, '%D %b %Y') AS Updated,l.DisapproveReason FROM timeadjust_requests l LEFT JOIN employees e ON l.EmpID = e.ID LEFT JOIN employees a ON l.ApprovedBy = a.ID LEFT JOIN roster_login_history r ON l.RosterID = r.ID WHERE l.ID <>0 AND (l.DateAdded BETWEEN '".date_format_Ymd($FromDate)."' AND '".date_format_Ymd($TillDate)."') AND e.ID = '".$_SESSION['UserID']."' ORDER BY l.Approved,l.ID DESC";
  ...
  <aside class="right-side">
    ...
    <section class="content-header">
      <h1> My Time Adjust Requests <small></small> </h1>
      <ol class="breadcrumb">
        <li><a href="Dashboard.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">My Time Adjust Requests</li>
      </ol>
    </section>
    <!-- Main content -->
      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">My Time Adjust Requests</h3>
			...
			<div class="row">
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" class="margin">
                        ...
                        <div class="form-group">
						 Employee:
						<select name="Employee" id="myselectdemo" disabled class="js-example-disabled-results form-control">
						  <option value="" >All Employees</option>
							...
							{
							echo '<option '.($_SESSION['UserID'] == $row['ID'] ? 'selected' : '').' value="'.$row['ID'].'">'.$row['EmpID'].' | '.$row['FirstName'].' '.$row['LastName'].'</option>';
							} 
							?>
						</select>
						</div>
		...
		?>
                    <tr>
					  ...
					  <td><?php echo $row["DisapproveReason"]; ?></td>
                    </tr>
                    <?php				
				}
			} 
