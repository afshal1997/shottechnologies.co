...
{			
		...
		if(isset($_POST["LoanRequestID"]))
			$LoanRequestID=trim($_POST["LoanRequestID"]);
		...
		$query = "SELECT ID FROM loans where EmpID = ".$EmployeeID." AND LoanType = '".$LoanType."' AND Status = 0";
		...
		else if($LoanRequestID == 0)
		{
			$msg='<div class="alert alert-danger alert-dismissable">
			<i class="fa fa-ban"></i>
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
			<b>Please select Loan Request Number.</b>
			</div>';
		}
	...
	if($msg=="")
	{
		...
		$query="INSERT INTO loans SET 
				...
				LoanReqID = '" . dbinput($LoanRequestID) . "',
				Code = '".dbinput($Code)."',
				Status = '".(int)$Status."',
		...
		$query="UPDATE loan_requests SET 
				LoanGranted = 1,
				DateModified = NOW() WHERE ID = '" . dbinput($LoanRequestID) . "'";
		mysql_query($query) or die (mysql_error());
...
<script src="http://ajax.googleapis.com/ajax/libs/jquery/1.11.2/jquery.min.js"></script>
		<script>
			...
			$("#EmployeeID").change(function () {
				...
				{
					$.ajax({			
							url: 'get_loanrequest.php?EmpID='+$("#EmployeeID").val(),
							success: function(data) {
								$("#LoanRequestID").html(data);
								$("#loanReqID").slideDown(); 
							},
            ...
            <div style="padding:15px;" class="box-primary">
              ...
              <div class="box-body">
				...
				<div id="loanReqID" style="display:none;" class="form-group">
					<label id="labelimp">Loan Requests:<span class="requiredStar">*</span></label>
						<div>
						<select name="LoanRequestID" id="LoanRequestID" class="form-control">
						</select>
						</div>
				</div>
